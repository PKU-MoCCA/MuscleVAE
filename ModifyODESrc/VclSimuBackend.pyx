...
cdef class World:
    ...
    cpdef loadBodyPos(self, np.ndarray np_id, np.ndarray np_pos):
        ...
        ## debug
        # print("np_id_buff.size", np_id_buff.size)
        ##
    ...
    # input
    # - np_id : np.ndarray in shape (num_body,) with dtype == np.uint64 for body pointer
    # - np_pos: np.ndarray in shape (num_body * 4, ) with dtype == np.float64
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef loadBodyPosODEFormat(self, np.ndarray np_id, np.ndarray np_pos):
        cdef np.ndarray np_id_buff = np.ascontiguousarray(np_id)
        cdef np.ndarray[np.float64_t, ndim = 1] np_pos_buff = np.ascontiguousarray(np_pos)
        cdef dBodyID * res_id = <dBodyID*> np_id_buff.data
        cdef const dReal * res_pos = <const dReal *> np_pos_buff.data
        ...
        while idx < cnt:
            b = res_id[idx]
            dBodySetPosition(b, res_pos[4 * idx], res_pos[4 * idx + 1], res_pos[4 * idx + 2])
            idx += 1
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef loadBodyQuatODEFormat(self, np.ndarray np_id, np.ndarray np_quat):
        cdef np.ndarray np_id_buff = np.ascontiguousarray(np_id)
        cdef np.ndarray[np.float64_t, ndim = 1] np_quat_buff = np.ascontiguousarray(np_quat)
        cdef dBodyID * res_id = <dBodyID*> np_id_buff.data
        cdef const dReal * res_quat = <const dReal *> np_quat_buff.data
        ...
        while idx < cnt:
            b = res_id[idx]
            dBodySetQuaternion(b, &res_quat[4 * idx])
    ...
    # input
    # - np_id  : np.ndarray in shape (num_body,) with dtype == np.uint64 for body pointer
    # - np_rot_mat: np.ndarray in shape (num_body * 12, ) with dtype == np.float64
    @cython.boundscheck(False)
    @cython.wraparound(False)
    cpdef loadBodyQuatFromMatrix(self, np.ndarray np_id, np.ndarray np_rot_mat):
        cdef np.ndarray np_id_buff = np.ascontiguousarray(np_id)
        cdef np.ndarray[np.float64_t, ndim = 1] np_rot_mat_buff = np.ascontiguousarray(np_rot_mat)
        cdef dBodyID * res_id = <dBodyID*> np_id_buff.data
        cdef const dReal * res_rot_mat = <const dReal *> np_rot_mat_buff.data
        cdef int idx = 0
        cdef int cnt = np_id_buff.size
        cdef dQuaternion q_ode
        while idx < cnt:
            b = res_id[idx]
            dQfromR(q_ode, &res_rot_mat[12 * idx])
            dBodySetQuaternion(b, q_ode)
    ...
    def getBallAndHingeAnchor1(self, np.ndarray np_id):
        ...
        while idx < cnt:
            ...
            # elif joint_type == dJointTypeFixed:
            #     dJointGetFixedAnchor(j, anc_ptr)
            else:
                raise NotImplementedError
    ...
    @cython.boundscheck(False)
    @cython.wraparound(False)
    def get_pd_control_torque_with_fatigue(self,
                              np.ndarray joint_id,
                              local_target_quat_in: np.ndarray,
                              kps_in: np.ndarray,
                              tor_lim_in: np.ndarray,
                              joint_capacity : np.ndarray):
        assert joint_id.dtype == np_size_t
        cdef int joint_count = joint_id.size
        cdef np.ndarray[np.float64_t, ndim=2] local_torque = np.zeros((joint_count, 3), dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=2] global_torque = np.zeros((joint_count, 3), dtype=np.float64)
        ...
        cdef np.ndarray[np.float64_t, ndim=1] kps = np.ascontiguousarray(kps_in, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] tor_lim = np.ascontiguousarray(tor_lim_in, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] joint_cap = np.ascontiguousarray(joint_capacity, dtype=np.float64)
        pd_control_batch_with_fatigue(
            <dJointID*> joint_id.data,
            joint_count,
            <const dReal*> local_target.data,
            <const dReal*> kps.data,
...
####------------------------naive math auxilary function----------------------------------###
    #
    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef ndarray_change_to_ode_mat_format(self,
                                        np.ndarray[np.float64_t, ndim = 1] mat, 
                                        dReal * res):
        """
        9 -> 12
        """
        cdef dReal * mat_from = <dReal *> mat.data
        res[0] = mat_from[0]
        res[1] = mat_from[1]
        res[2] = mat_from[2]
        res[3] = 0.0
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef dVector3_change_to_ode_vec_format(self,
                                        dVector3 vec_from, 
                                        dReal * res):
        """
        4 -> 3
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef ndarray_change_from_ode_mat_format(self,
                                            np.ndarray[np.float64_t, ndim = 1] mat, 
                                            dReal * res):
        """
        12 -> 9
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef dMatrix3_change_from_ode_mat_format(self,
                                            dMatrix3 mat_from, 
                                            dReal * res):
        """
        12 -> 9
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef dMatrix3_copy_to_dMatrix3_format(self,
                                        dMatrix3 mat_from, 
                                        dReal * res):
        """
        12 -> 12
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef ode_mat_copy_to_dMatrix3_format(self,
                                        dReal * mat_from, 
                                        dReal * res):
        """
        9 -> 12
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef I3_dMatrix3_copy_to_dMatrix3_format(self,
                                            dReal * res):
        """
        -> 12
        """
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef cython_vec1_dot_vec2(self,
                            dReal * vec1,
                            dReal * vec2):
        cdef dReal res
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef cython_mat1_matmul_mat2_T(self,
                                dReal * res,
                                dReal * mat1,
                                dReal * mat2):
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef cython_mat1_T_matmul_mat2(self,
                                dReal * res,
                                dReal * mat1,
                                dReal * mat2):
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef cython_mat1_matmul_mat2(self,
                                dReal * res,
                                dReal * mat1,
                                dReal * mat2):
...
####-------------------end naive math auxilary function----------------------------------###
    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_adjust_joint_kd_same(self, 
                                    np.ndarray[np.uint64_t, ndim = 1] joint_cid,
                                    np.ndarray[np.float64_t, ndim = 1] input_kd_coff):
        cdef np.ndarray[np.uint64_t, ndim = 1] joint_cid_buf = np.ascontiguousarray(joint_cid, dtype=np.uint64)
        cdef np.ndarray[np.float64_t, ndim = 1] input_kd_coff_buf = np.ascontiguousarray(input_kd_coff, dtype=np.float64)
        cdef int joint_cnt = joint_cid_buf.size
        cdef int idx = 0
        while idx < joint_cnt :
            dJointSetKd(<dJointID>joint_cid_buf[idx], input_kd_coff_buf[idx], input_kd_coff_buf[idx], input_kd_coff_buf[idx])
            idx += 1
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_add_muscle_anchor_forces_to_body(self, 
                                        np.ndarray[np.uint64_t, ndim = 1] anchor_add_force_bd_cid,
                                        np.ndarray[np.float64_t, ndim = 1] global_displacement, 
                                        np.ndarray[np.float64_t, ndim = 1] global_force):
        cdef np.ndarray[np.float64_t, ndim = 1] displacement_buf = np.ascontiguousarray(global_displacement, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] force_buf = np.ascontiguousarray(global_force, dtype=np.float64)
        cdef np.ndarray[np.uint64_t, ndim = 1] id_buf = np.ascontiguousarray(anchor_add_force_bd_cid, dtype=np.uint64)

        cdef dReal * displacement_res = <dReal *> displacement_buf.data
        cdef dReal * force_res = <dReal *> force_buf.data
        cdef dBodyID * id_res = <dBodyID *> id_buf.data
        cdef int cnt = id_buf.size 
        cdef int idx = 0
        cdef dBodyID body_id = NULL
        while idx < cnt:
            body_id = <dBodyID> id_buf[idx]
            dBodyAddForceAtPos(body_id, force_buf[idx *3 + 0], force_buf[idx *3 + 1], force_buf[idx *3 + 2], displacement_buf[idx *3 + 0], displacement_buf[idx *3 + 1], displacement_buf[idx *3 + 2])
            idx += 1
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    cdef only_cython_anchor_local_pos_to_global_pos_geom_base(self, 
                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                                            np.ndarray[np.uint64_t, ndim=1] which_body0_geom0, 
                                                            np.ndarray[np.uint64_t, ndim=1] which_body1_geom0,  
                                                            np.ndarray[np.float64_t, ndim=1] body0_weight,
                                                            np.ndarray[np.float64_t, ndim=1] geom0_body0_offset,
                                                            unsigned long long body0_geom0_gid,
                                                            dReal* anchor_global_pos_ptr): 
        cdef np.ndarray[np.uint64_t, ndim = 1] body0_geom0_buf = np.ascontiguousarray(which_body0_geom0, dtype=np.uint64)
        cdef np.ndarray[np.uint64_t, ndim = 1] body1_geom0_buf = np.ascontiguousarray(which_body1_geom0, dtype=np.uint64)

        cdef np.ndarray[np.float64_t, ndim = 1] local_pos_of_body0_buf_ori = np.ascontiguousarray(local_pos_of_body0, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] local_pos_of_body1_buf_ori = np.ascontiguousarray(local_pos_of_body1, dtype=np.float64)
        cdef dReal* local_pos_of_body0_buf = <dReal*>local_pos_of_body0_buf_ori.data
        cdef dReal* local_pos_of_body1_buf = <dReal*>local_pos_of_body1_buf_ori.data
        
        cdef np.ndarray[np.float64_t, ndim = 1] body0_weight_buf = np.ascontiguousarray(body0_weight, dtype=np.float64)

        cdef int idx = 0
        cdef int anchor_size = local_pos_of_body0.size // 3

        cdef dReal* tmp_bd0_geom0_rot 
        cdef dReal* tmp_bd0_geom0_pos
        cdef dReal* tmp_bd1_geom0_rot
        cdef dReal* tmp_bd1_geom0_pos

        while idx < anchor_size:
            if body0_weight_buf[idx] - 1.0 < 1e-8 and body0_weight_buf[idx] - 1.0 > -1e-8:
                tmp_bd0_geom0_rot = <dReal *> dGeomGetRotation(<dGeomID> body0_geom0_buf[idx])
                tmp_bd0_geom0_pos = <dReal *> dGeomGetPosition(<dGeomID> body0_geom0_buf[idx])
                anchor_global_pos_ptr[3 * idx + 0] = self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+0, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[0]
                anchor_global_pos_ptr[3 * idx + 1] = self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+4, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[1]
                anchor_global_pos_ptr[3 * idx + 2] = self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+8, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[2]
                
                if body0_geom0_buf[idx] == body0_geom0_gid:
                    anchor_global_pos_ptr[3 * idx + 0] += geom0_body0_offset[0]
                    anchor_global_pos_ptr[3 * idx + 1] += geom0_body0_offset[1]
                    anchor_global_pos_ptr[3 * idx + 2] += geom0_body0_offset[2]

            else:
                tmp_bd0_geom0_rot = <dReal *> dGeomGetRotation(<dGeomID> body0_geom0_buf[idx])
                tmp_bd0_geom0_pos = <dReal *> dGeomGetPosition(<dGeomID> body0_geom0_buf[idx])
                tmp_bd1_geom0_rot = <dReal *> dGeomGetRotation(<dGeomID> body1_geom0_buf[idx])
                tmp_bd1_geom0_pos = <dReal *> dGeomGetPosition(<dGeomID> body1_geom0_buf[idx])

                anchor_global_pos_ptr[3 * idx + 0] = (self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+0, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[0]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 1] = (self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+4, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[1]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 2] = (self.cython_vec1_dot_vec2(tmp_bd0_geom0_rot+8, local_pos_of_body0_buf+3*idx) + tmp_bd0_geom0_pos[2]) * body0_weight_buf[idx]

                anchor_global_pos_ptr[3 * idx + 0] += (self.cython_vec1_dot_vec2(tmp_bd1_geom0_rot+0, local_pos_of_body1_buf+3*idx) + tmp_bd1_geom0_pos[0]) * (1 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 1] += (self.cython_vec1_dot_vec2(tmp_bd1_geom0_rot+4, local_pos_of_body1_buf+3*idx) + tmp_bd1_geom0_pos[1]) * (1 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 2] += (self.cython_vec1_dot_vec2(tmp_bd1_geom0_rot+8, local_pos_of_body1_buf+3*idx) + tmp_bd1_geom0_pos[2]) * (1 - body0_weight_buf[idx])

                # deal with geom0 towards body0 offset
                if body0_geom0_buf[idx] == body0_geom0_gid:
                    anchor_global_pos_ptr[3 * idx + 0] += geom0_body0_offset[0] * body0_weight_buf[idx]
                    anchor_global_pos_ptr[3 * idx + 1] += geom0_body0_offset[1] * body0_weight_buf[idx]
                    anchor_global_pos_ptr[3 * idx + 2] += geom0_body0_offset[2] * body0_weight_buf[idx]
                if body1_geom0_buf[idx] == body0_geom0_gid:
                    anchor_global_pos_ptr[3 * idx + 0] += geom0_body0_offset[0] * (1.0 - body0_weight_buf[idx])
                    anchor_global_pos_ptr[3 * idx + 1] += geom0_body0_offset[1] * (1.0 - body0_weight_buf[idx])
                    anchor_global_pos_ptr[3 * idx + 2] += geom0_body0_offset[2] * (1.0 - body0_weight_buf[idx])
            idx += 1
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def new_anchor_local_pos_to_global_pos_geom(self, 
                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                            np.ndarray[np.uint64_t, ndim=1] which_body0_geom0, 
                                            np.ndarray[np.uint64_t, ndim=1] which_body1_geom0,  
        ...
        cdef np.ndarray[np.float64_t, ndim = 1] anchor_global_pos = np.zeros(anchor_size * 3, dtype=np.float64)
        cdef dReal * anchor_global_pos_ptr = <dReal *> anchor_global_pos.data
        self.only_cython_anchor_local_pos_to_global_pos_geom_base(local_pos_of_body0, local_pos_of_body1, which_body0_geom0, which_body1_geom0, body0_weight, geom0_body0_offset, body0_geom0_gid, anchor_global_pos_ptr) # 
        
        return anchor_global_pos

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_update_anchor_pos(self, 
                                np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                np.ndarray[np.uint64_t, ndim=1] which_body0_geom0, 
                                np.ndarray[np.uint64_t, ndim=1] which_body1_geom0,  
                                ...
                                np.ndarray[np.float64_t, ndim=1] geom0_body0_offset,
                                unsigned long long body0_geom0_gid,
                                np.ndarray[np.int32_t, ndim=1]muscle_hold_num):
        cdef np.ndarray[np.int32_t, ndim = 1] muscle_hold_num_buf = np.ascontiguousarray(muscle_hold_num, dtype=np.int32)
        cdef int anchor_size = local_pos_of_body0.size // 3
        cdef int muscle_size = muscle_hold_num.size
        cdef int muscle_idx = 0
        cdef int tmp_idx = 0
        ...
        cdef np.ndarray[np.float64_t, ndim = 1] anchor_global_pos = np.zeros(anchor_size * 3, dtype=np.float64)
        cdef dReal * anchor_global_pos_ptr = <dReal *> anchor_global_pos.data
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_length = np.zeros(muscle_size, dtype=np.float64)
        cdef dReal * muscle_length_ptr = <dReal *> muscle_length.data
        self.only_cython_anchor_local_pos_to_global_pos_geom_base(local_pos_of_body0, local_pos_of_body1, which_body0_geom0, which_body1_geom0, body0_weight, geom0_body0_offset, body0_geom0_gid, anchor_global_pos_ptr) # 
        
        while muscle_idx < muscle_size :
            tmp_idx = 0
            while tmp_idx < muscle_hold_num_buf[muscle_idx] - 1:
                tmp0 = anchor_global_pos_ptr[(anchor_idx + tmp_idx) * 3 + 0] - anchor_global_pos_ptr[(anchor_idx + tmp_idx + 1) * 3 + 0]
                tmp1 = anchor_global_pos_ptr[(anchor_idx + tmp_idx) * 3 + 1] - anchor_global_pos_ptr[(anchor_idx + tmp_idx + 1) * 3 + 1]
                tmp2 = anchor_global_pos_ptr[(anchor_idx + tmp_idx) * 3 + 2] - anchor_global_pos_ptr[(anchor_idx + tmp_idx + 1) * 3 + 2]
                muscle_length_ptr[muscle_idx] += sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 

                tmp_idx += 1

            
            anchor_idx += muscle_hold_num_buf[muscle_idx]
            muscle_idx += 1
            

        return anchor_global_pos, muscle_length
        
    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def anchor_local_pos_to_global_pos_geom(self, 
                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                            np.ndarray[np.uint64_t, ndim=1] which_body0_geom0, 
                                            np.ndarray[np.uint64_t, ndim=1] which_body1_geom0,  
        ...
        cdef np.ndarray[np.float64_t, ndim = 1] local_pos_of_body0_buf = np.ascontiguousarray(local_pos_of_body0, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] local_pos_of_body1_buf = np.ascontiguousarray(local_pos_of_body1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] body0_weight_buf = np.ascontiguousarray(body0_weight, dtype=np.float64)

        cdef int idx = 0
        cdef int anchor_size = local_pos_of_body0_buf.size // 3

        cdef np.ndarray[np.float64_t, ndim = 1] anchor_global_pos = np.zeros(anchor_size * 3, dtype=np.float64)
        cdef dReal * res = <dReal *> anchor_global_pos.data
        ...
        while idx < anchor_size:
            if body0_weight_buf[idx] - 1.0 < 1e-8 and body0_weight_buf[idx] - 1.0 > -1e-8:
                tmp_bd0_geom0_rot = <dReal *> dGeomGetRotation(<dGeomID> body0_geom0_buf[idx])
                tmp_bd0_geom0_pos = <dReal *> dGeomGetPosition(<dGeomID> body0_geom0_buf[idx])
                res[3 * idx + 0] = tmp_bd0_geom0_rot[0] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[1] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[2] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[0]
                res[3 * idx + 1] = tmp_bd0_geom0_rot[4] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[5] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[6] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[1]
                res[3 * idx + 2] = tmp_bd0_geom0_rot[8] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[9] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[10] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[2]
                ...
                res[3 * idx + 0] = (tmp_bd0_geom0_rot[0] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[1] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[2] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[0]) * body0_weight_buf[idx]
                res[3 * idx + 1] = (tmp_bd0_geom0_rot[4] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[5] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[6] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[1]) * body0_weight_buf[idx]
                res[3 * idx + 2] = (tmp_bd0_geom0_rot[8] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[9] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[10] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[2]) * body0_weight_buf[idx]

                res[3 * idx + 0] += (tmp_bd1_geom0_rot[0] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[1] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[2] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[0]) * (1 - body0_weight_buf[idx])
                res[3 * idx + 1] += (tmp_bd1_geom0_rot[4] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[5] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[6] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[1]) * (1 - body0_weight_buf[idx])
                res[3 * idx + 2] += (tmp_bd1_geom0_rot[8] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[9] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[10] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[2]) * (1 - body0_weight_buf[idx])
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def geom_rotation_in_matrix_format(self,
                                        np.ndarray[np.uint64_t, ndim=1] body_geom_id):
        """
        there is a similar function in ODEMuscleCharacter.body_info.get_geom_rot()
        but this function only return each body's geom0 rotation
        """
        cdef np.ndarray[np.uint64_t, ndim = 1] body_geom_id_buf = np.ascontiguousarray(body_geom_id, dtype=np.uint64)
        cdef int body_cnt = body_geom_id_buf.size
        cdef np.ndarray[np.float64_t, ndim = 1] body_geom_rotation_matrix = np.zeros(body_cnt * 9, dtype=np.float64)
        cdef dReal* body_geom_rotation_matrix_ptr = <dReal*> body_geom_rotation_matrix.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_ode_mat = np.zeros(12, dtype=np.float64)
        cdef dReal* tmp_ode_mat_ptr = <dReal*>  tmp_ode_mat.data
        cdef int idx = 0

        while idx < body_cnt:
            tmp_ode_mat_ptr = <dReal *> dGeomGetRotation(<dGeomID> body_geom_id_buf[idx])
            self.dMatrix3_change_from_ode_mat_format(tmp_ode_mat_ptr, body_geom_rotation_matrix_ptr+9*idx)
            idx += 1 
        return body_geom_rotation_matrix   

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def geom_position(self,
                    np.ndarray[np.uint64_t, ndim=1] body_geom_id):
        """
        there is a similar function in ODEMuscleCharacter.body_info.get_geom_pos()
        but this function only return each body's geom0 position
        """
        cdef np.ndarray[np.uint64_t, ndim = 1] body_geom_id_buf = np.ascontiguousarray(body_geom_id, dtype=np.uint64)
        cdef int body_cnt = body_geom_id_buf.size
        cdef np.ndarray[np.float64_t, ndim = 1] body_geom_position = np.zeros(body_cnt * 3, dtype=np.float64)
        cdef dReal* body_geom_position_ptr = <dReal*> body_geom_position.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_ode_vec = np.zeros(4, dtype=np.float64)
        cdef dReal* tmp_ode_vec_ptr = <dReal*>  tmp_ode_vec.data
        cdef int idx = 0

        while idx < body_cnt:
            tmp_ode_vec_ptr = <dReal *> dGeomGetPosition(<dGeomID> body_geom_id_buf[idx])
            self.dVector3_change_to_ode_vec_format(tmp_ode_vec_ptr, body_geom_position_ptr+3*idx)
            idx += 1 
        return body_geom_position                     

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def anchor_local_pos_to_global_pos_geom_with_output_absolute_muscle_len(self, 
                                                                            np.ndarray[np.uint64_t, ndim=1] body_geom_id, 
                                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                                                            np.ndarray[np.uint64_t, ndim=1] which_body0_geom0, 
        ...
        cdef int anchor_idx = 0
        cdef int tmp_idx = 0
        cdef int muscle_size = muscle_hold_num_buf.size

        cdef np.ndarray[np.float64_t, ndim = 1] anchor_global_pos = np.zeros(anchor_size * 3, dtype=np.float64)
        cdef dReal * anchor_global_pos_ptr = <dReal *> anchor_global_pos.data
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_length = np.zeros(muscle_size, dtype=np.float64)
        cdef dReal * res = <dReal *> muscle_length.data
        ...
        while idx < anchor_size:
            if body0_weight_buf[idx] - 1.0 < 1e-8 and body0_weight_buf[idx] - 1.0 > -1e-8:
                tmp_bd0_geom0_rot = <dReal *> dGeomGetRotation(<dGeomID> body0_geom0_buf[idx])
                tmp_bd0_geom0_pos = <dReal *> dGeomGetPosition(<dGeomID> body0_geom0_buf[idx])
                anchor_global_pos_ptr[3 * idx + 0] = tmp_bd0_geom0_rot[0] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[1] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[2] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[0]
                anchor_global_pos_ptr[3 * idx + 1] = tmp_bd0_geom0_rot[4] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[5] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[6] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[1]
                anchor_global_pos_ptr[3 * idx + 2] = tmp_bd0_geom0_rot[8] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[9] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[10] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[2]
                ...
                anchor_global_pos_ptr[3 * idx + 0] = (tmp_bd0_geom0_rot[0] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[1] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[2] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[0]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 1] = (tmp_bd0_geom0_rot[4] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[5] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[6] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[1]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 2] = (tmp_bd0_geom0_rot[8] * local_pos_of_body0_buf[3 *idx + 0] + tmp_bd0_geom0_rot[9] * local_pos_of_body0_buf[3 *idx + 1] + tmp_bd0_geom0_rot[10] * local_pos_of_body0_buf[3 *idx + 2] + tmp_bd0_geom0_pos[2]) * body0_weight_buf[idx]

                anchor_global_pos_ptr[3 * idx + 0] += (tmp_bd1_geom0_rot[0] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[1] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[2] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[0]) * (1 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 1] += (tmp_bd1_geom0_rot[4] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[5] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[6] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[1]) * (1 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 2] += (tmp_bd1_geom0_rot[8] * local_pos_of_body1_buf[3 *idx + 0] + tmp_bd1_geom0_rot[9] * local_pos_of_body1_buf[3 *idx + 1] + tmp_bd1_geom0_rot[10] * local_pos_of_body1_buf[3 *idx + 2] + tmp_bd1_geom0_pos[2]) * (1 - body0_weight_buf[idx])
        ...
        while muscle_idx < muscle_size :
            ...
            while tmp_idx < muscle_hold_num_buf[muscle_idx] - 1:
                ...
                tmp1 = anchor_global_pos_ptr[(anchor_idx + tmp_idx) * 3 + 1] - anchor_global_pos_ptr[(anchor_idx + tmp_idx + 1) * 3 + 1]
                tmp2 = anchor_global_pos_ptr[(anchor_idx + tmp_idx) * 3 + 2] - anchor_global_pos_ptr[(anchor_idx + tmp_idx + 1) * 3 + 2]
                res[muscle_idx] += sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_scale_calc_muscle_len(self,
                                    np.ndarray[np.float64_t, ndim=1]input_calc_muscle_len,
                                    np.ndarray[np.float64_t, ndim=1]muscle_properties1, 
                                    np.ndarray[np.float64_t, ndim=1]muscle_properties2, 
                                    np.ndarray[np.float64_t, ndim=1]muscle_origin_length_calculated):
        cdef np.ndarray[np.float64_t, ndim=1] input_calc_muscle_len_buf = np.ascontiguousarray(input_calc_muscle_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_origin_length_calculated_buf = np.ascontiguousarray(muscle_origin_length_calculated, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lm0_buf = np.ascontiguousarray(muscle_properties1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lt0_buf = np.ascontiguousarray(muscle_properties2, dtype=np.float64)

        cdef int muscle_num = input_calc_muscle_len.size
        cdef int muscle_idx = 0

        cdef np.ndarray[np.float64_t, ndim = 1] scaled_muscle_len = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * res = <dReal *> scaled_muscle_len.data

        while muscle_idx < muscle_num :
            res[muscle_idx] = input_calc_muscle_len_buf[muscle_idx]
            res[muscle_idx] /= muscle_origin_length_calculated_buf[muscle_idx]
            res[muscle_idx] -= lt0_buf[muscle_idx]
            res[muscle_idx] /= lm0_buf[muscle_idx]
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_update_anchor_pos_with_input_body_rot_pos(self, 
                                                            np.ndarray[np.float64_t, ndim=1] input_body_rot, 
                                                            np.ndarray[np.float64_t, ndim=1] input_body_pos, 
                                                            np.ndarray[np.float64_t, ndim=1] body_toward_geom_rot, 
                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body0, 
                                                            np.ndarray[np.float64_t, ndim=1] local_pos_of_body1, 
                                                            np.ndarray[np.int32_t, ndim=1] which_body0, 
                                                            np.ndarray[np.int32_t, ndim=1] which_body1,  
                                                            np.ndarray[np.float64_t, ndim=1] body0_weight,
                                                            np.ndarray[np.int32_t, ndim=1]muscle_hold_num):
        """
        input_body_rot: [4*body_size] quaternion
        """ 
        cdef dReal* input_body_rot_buf = <dReal *> input_body_rot.data
        cdef np.ndarray[np.float64_t, ndim = 1] input_body_pos_buf = np.ascontiguousarray(input_body_pos, dtype=np.float64)
        cdef dReal* body_toward_geom_rot_buf = <dReal *> body_toward_geom_rot.data
        cdef np.ndarray[np.int32_t, ndim = 1] body0_buf = np.ascontiguousarray(which_body0, dtype=np.int32)
        cdef np.ndarray[np.int32_t, ndim = 1] body1_buf = np.ascontiguousarray(which_body1, dtype=np.int32)
        ...
        cdef int muscle_idx = 0
        cdef int anchor_idx = 0
        cdef int body_size = input_body_pos_buf.size // 3
        cdef int tmp_idx = 0
        cdef int muscle_size = muscle_hold_num_buf.size

        cdef np.ndarray[np.float64_t, ndim = 1] input_geom_rot = np.zeros(body_size * 9, dtype=np.float64)
        cdef dReal * input_geom_rot_buf = <dReal * > input_geom_rot.data
        ...
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_quat_adjusted_np = np.zeros(4, dtype=np.float64)
        cdef dReal * tmp_quat_adjusted = <dReal *> tmp_quat_adjusted_np.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_bd_rot_np = np.zeros(9, dtype=np.float64)
        cdef dReal * tmp_bd_rot_np_ptr = <dReal *> tmp_bd_rot_np.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_bd_rot = np.zeros(12, dtype=np.float64)
        cdef dReal * tmp_bd_rot_ptr = <dReal *> tmp_bd_rot.data
        
        
        cdef int tmp_bd0_geom0_idx 
        cdef int tmp_bd1_geom0_idx
        ...
        while idx < body_size :
            tmp_quat_adjusted[0] = input_body_rot_buf[3+4*idx]
            tmp_quat_adjusted[1] = input_body_rot_buf[0+4*idx]
            tmp_quat_adjusted[2] = input_body_rot_buf[1+4*idx]
            tmp_quat_adjusted[3] = input_body_rot_buf[2+4*idx]

            dRfromQ(tmp_bd_rot_ptr, tmp_quat_adjusted)
            self.dMatrix3_change_from_ode_mat_format(tmp_bd_rot_ptr, tmp_bd_rot_np_ptr)

            self.cython_mat1_matmul_mat2(input_geom_rot_buf+9*idx, tmp_bd_rot_np_ptr, body_toward_geom_rot_buf+9*idx)
            idx += 1

        idx = 0
        while idx < anchor_size:
            if body0_weight_buf[idx] - 1.0 < 1e-8 and body0_weight_buf[idx] - 1.0 > -1e-8:
                tmp_bd0_geom0_idx = body0_buf[idx]
                anchor_global_pos_ptr[3 * idx + 0] = input_geom_rot_buf[9*tmp_bd0_geom0_idx+0] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+1] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+2] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+0]
                anchor_global_pos_ptr[3 * idx + 1] = input_geom_rot_buf[9*tmp_bd0_geom0_idx+3] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+4] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+5] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+1]
                anchor_global_pos_ptr[3 * idx + 2] = input_geom_rot_buf[9*tmp_bd0_geom0_idx+6] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+7] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+8] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+2]
            
            else:
                tmp_bd0_geom0_idx = body0_buf[idx]
                tmp_bd1_geom0_idx = body1_buf[idx]

                anchor_global_pos_ptr[3 * idx + 0] = (input_geom_rot_buf[9*tmp_bd0_geom0_idx+0] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+1] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+2] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+0]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 1] = (input_geom_rot_buf[9*tmp_bd0_geom0_idx+3] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+4] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+5] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+1]) * body0_weight_buf[idx]
                anchor_global_pos_ptr[3 * idx + 2] = (input_geom_rot_buf[9*tmp_bd0_geom0_idx+6] * local_pos_of_body0_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+7] * local_pos_of_body0_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd0_geom0_idx+8] * local_pos_of_body0_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd0_geom0_idx+2]) * body0_weight_buf[idx]
            
                anchor_global_pos_ptr[3 * idx + 0] += (input_geom_rot_buf[9*tmp_bd1_geom0_idx+0] * local_pos_of_body1_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+1] * local_pos_of_body1_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+2] * local_pos_of_body1_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd1_geom0_idx+0]) * (1.0 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 1] += (input_geom_rot_buf[9*tmp_bd1_geom0_idx+3] * local_pos_of_body1_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+4] * local_pos_of_body1_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+5] * local_pos_of_body1_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd1_geom0_idx+1]) * (1.0 - body0_weight_buf[idx])
                anchor_global_pos_ptr[3 * idx + 2] += (input_geom_rot_buf[9*tmp_bd1_geom0_idx+6] * local_pos_of_body1_buf[3 *idx + 0] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+7] * local_pos_of_body1_buf[3 *idx + 1] + input_geom_rot_buf[9*tmp_bd1_geom0_idx+8] * local_pos_of_body1_buf[3 *idx + 2] + input_body_pos_buf[3*tmp_bd1_geom0_idx+2]) * (1.0 - body0_weight_buf[idx])
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def cython_simple_forward_kinematics_with_local_action_target(self, 
                                                                np.ndarray[np.float64_t, ndim=1] root_body_pos, 
                                                                np.ndarray[np.float64_t, ndim=1] root_body_rot, 
                                                                np.ndarray[np.float64_t, ndim=1] action_local_target,
                                                                np.ndarray[np.int32_t, ndim=1] which_parent,  
                                                                np.ndarray[np.float64_t, ndim=1] joint_tpose,
                                                                np.ndarray[np.float64_t, ndim=1] body_com_tpose,
                                                                np.ndarray[np.int32_t, ndim=1] child_body_to_joint):
        """
        @input: local action, a target pose  (not offset) quaternion format
        @output: new_body_pos, 4 * num_body
        @output: new_body_rot, 12 * num_body (rotation matrix)
        """
        cdef dReal* root_body_pos_ptr = <dReal *> root_body_pos.data
        cdef dReal* root_body_rot_ptr = <dReal *> root_body_rot.data
        cdef int body_size = body_com_tpose.size // 3
        cdef np.ndarray[np.float64_t, ndim = 1] new_body_pos = np.zeros(body_size * 4, dtype=np.float64)
        cdef dReal * new_body_pos_ptr = <dReal *> new_body_pos.data
        cdef np.ndarray[np.float64_t, ndim = 1] new_body_rot = np.zeros(body_size * 12, dtype=np.float64)
        cdef dReal * new_body_rot_ptr = <dReal *> new_body_rot.data
        self.only_cython_simple_forward_kinematics_with_local_action_target(root_body_pos_ptr, root_body_rot_ptr, action_local_target, which_parent, joint_tpose, body_com_tpose, child_body_to_joint, new_body_pos_ptr, new_body_rot_ptr)
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    cdef only_cython_simple_forward_kinematics_with_local_action_target(self, 
                                                                dReal* root_body_pos_ptr, 
                                                                dReal* root_body_rot_ptr, 
                                                                np.ndarray[np.float64_t, ndim=1] action_local_target,
                                                                np.ndarray[np.int32_t, ndim=1] which_parent,  
                                                                np.ndarray[np.float64_t, ndim=1] joint_tpose,
                                                                np.ndarray[np.float64_t, ndim=1] body_com_tpose,
                                                                np.ndarray[np.int32_t, ndim=1] child_body_to_joint,
                                                                dReal * new_body_pos_ptr,
                                                                dReal * new_body_rot_ptr):
        """
        local action : a target pose  (not offset) scipy quaternion format
        """
        cdef np.ndarray[np.int32_t, ndim = 1] which_parent_buf = np.ascontiguousarray(which_parent, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim = 1] joint_tpose_buf = np.ascontiguousarray(joint_tpose, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] body_com_tpose_buf = np.ascontiguousarray(body_com_tpose, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim = 1] child_body_to_joint_buf = np.ascontiguousarray(child_body_to_joint, dtype=np.int32)


        ## in a format of quaternion
        cdef dReal * action_local_target_ptr = <dReal *> action_local_target.data

        cdef int idx = 0
        cdef int body_size = body_com_tpose_buf.size // 3  

        cdef np.ndarray[np.float64_t, ndim = 1] tmp_bd_rot_np = np.zeros(12, dtype=np.float64)
        cdef dReal* tmp_bd_rot = <dReal* >tmp_bd_rot_np.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_bd_parent_rot_np = np.zeros(12, dtype=np.float64)
        cdef dReal* tmp_bd_parent_rot = <dReal* >tmp_bd_parent_rot_np.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_bd_pos_np = np.zeros(4, dtype=np.float64)
        cdef dReal* tmp_bd_pos = <dReal* >tmp_bd_pos_np.data

        cdef np.ndarray[np.float64_t, ndim = 1] small_matrix0 = np.zeros(9, dtype=np.float64)
        cdef dReal* tmp_small_rot0 = <dReal *>small_matrix0.data
        cdef np.ndarray[np.float64_t, ndim = 1] small_matrix1 = np.zeros(9, dtype=np.float64)
        cdef dReal* tmp_small_rot1 = <dReal *>small_matrix1.data
        cdef np.ndarray[np.float64_t, ndim = 1] small_matrix2 = np.zeros(9, dtype=np.float64)
        cdef dReal* tmp_small_rot2 = <dReal *>small_matrix2.data

        cdef np.ndarray[np.float64_t, ndim = 1] tmp_quat_np = np.zeros(4, dtype=np.float64)
        cdef dQuaternion tmp_quat = <dQuaternion> tmp_quat_np.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_quat_adjusted_np = np.zeros(4, dtype=np.float64)
        cdef dQuaternion tmp_quat_adjusted = <dQuaternion> tmp_quat_adjusted_np.data

        cdef dReal tmp0
        cdef dReal tmp1
        cdef dReal tmp2

        cdef np.ndarray[np.float64_t, ndim = 1] small_vec = np.zeros(4, dtype=np.float64)
        cdef dReal* tmp_small_vec = <dReal *>small_vec.data
        cdef dReal tmp_norm


        idx = 1
        self.ode_mat_copy_to_dMatrix3_format(root_body_rot_ptr, new_body_rot_ptr)
        new_body_pos_ptr[0] = root_body_pos_ptr[0]
        new_body_pos_ptr[1] = root_body_pos_ptr[1]
        new_body_pos_ptr[2] = root_body_pos_ptr[2]
        new_body_pos_ptr[3] = 0.0
        while idx < body_size:
            tmp_quat = <dQuaternion> (action_local_target_ptr + 4 * (idx - 1))
            ## take consideration about the quat order!
            tmp_quat_adjusted[0] = tmp_quat[3]
            tmp_quat_adjusted[1] = tmp_quat[0]
            tmp_quat_adjusted[2] = tmp_quat[1]
            tmp_quat_adjusted[3] = tmp_quat[2]

            dRfromQ(tmp_bd_rot, tmp_quat_adjusted)
            self.dMatrix3_change_from_ode_mat_format(tmp_bd_rot, tmp_small_rot2)

            self.dMatrix3_change_from_ode_mat_format(<dReal *> (new_body_rot_ptr + 12 * which_parent_buf[idx] ), tmp_small_rot1)
            ## new child orientation tmp0 = tmp1 @ tmp2
            self.cython_mat1_matmul_mat2(tmp_small_rot0, tmp_small_rot1, tmp_small_rot2)
            
            self.ode_mat_copy_to_dMatrix3_format(tmp_small_rot0, <dReal*> (new_body_rot_ptr + 12 * idx))

            tmp_small_vec[0] = joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 0] - body_com_tpose_buf[3 * which_parent_buf[idx] + 0]
            tmp_small_vec[1] = joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 1] - body_com_tpose_buf[3 * which_parent_buf[idx] + 1]
            tmp_small_vec[2] = joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 2] - body_com_tpose_buf[3 * which_parent_buf[idx] + 2]

            tmp0 = self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * which_parent_buf[idx] + 0), tmp_small_vec)
            tmp1 = self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * which_parent_buf[idx] + 4), tmp_small_vec)
            tmp2 = self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * which_parent_buf[idx] + 8), tmp_small_vec)

            tmp_small_vec[0] = -joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 0] + body_com_tpose_buf[3 * idx + 0]
            tmp_small_vec[1] = -joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 1] + body_com_tpose_buf[3 * idx + 1]
            tmp_small_vec[2] = -joint_tpose_buf[3 * child_body_to_joint_buf[idx] + 2] + body_com_tpose_buf[3 * idx + 2]

            tmp0 += self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * idx + 0), tmp_small_vec)
            tmp1 += self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * idx + 4), tmp_small_vec)
            tmp2 += self.cython_vec1_dot_vec2(<dReal*> (new_body_rot_ptr + 12 * idx + 8), tmp_small_vec)

            new_body_pos_ptr[0 + 4 * idx] = new_body_pos_ptr[0 + 4 * which_parent_buf[idx]] + tmp0
            new_body_pos_ptr[1 + 4 * idx] = new_body_pos_ptr[1 + 4 * which_parent_buf[idx]] + tmp1
            new_body_pos_ptr[2 + 4 * idx] = new_body_pos_ptr[2 + 4 * which_parent_buf[idx]] + tmp2
            new_body_pos_ptr[3 + 4 * idx] = 0.0
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_just_get_anchor_active_force(self, 
                                            dReal gamma, 
                                            np.ndarray[np.float64_t, ndim=1]muscle_properties0, 
                                            np.ndarray[np.float64_t, ndim=1]muscle_properties1, 
                                            np.ndarray[np.float64_t, ndim=1]muscle_properties2, 
                                            np.ndarray[np.float64_t, ndim=1]anchor_pos, 
                                            np.ndarray[np.int32_t, ndim=1]muscle_hold_anchor_num, 
                                            np.ndarray[np.float64_t, ndim=1]muscle_origin_length_calculated):
        cdef np.ndarray[np.float64_t, ndim=1] anchor_global_pos_buf = np.ascontiguousarray(anchor_pos, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim=1] hold_num_buf = np.ascontiguousarray(muscle_hold_anchor_num, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] f0_buf = np.ascontiguousarray(muscle_properties0, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lm0_buf = np.ascontiguousarray(muscle_properties1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lt0_buf = np.ascontiguousarray(muscle_properties2, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_origin_length_calculated_buf = np.ascontiguousarray(muscle_origin_length_calculated, dtype=np.float64)

        cdef int anchor_num = anchor_global_pos_buf.size // 3
        cdef int muscle_num = hold_num_buf.size

        cdef np.ndarray[np.float64_t, ndim = 1] anchor_active_force = np.zeros(anchor_num * 3, dtype=np.float64)
        cdef dReal * res = <dReal *> anchor_active_force.data
        ...
        cdef dReal tmp4
        cdef dReal tmp5
        cdef dReal tmp_norm0
        cdef dReal tmp_norm1
        cdef dReal tmp_muscle_len
        cdef dReal tmp_muscle_amplitude

        while muscle_idx < muscle_num :
            tmp_idx = 0
            tmp_muscle_len = 0
            while tmp_idx < hold_num_buf[muscle_idx]:
                if tmp_idx == 0 :
                    tmp0 = anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 0] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 0]
                    tmp1 = anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 1] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 1]
                    tmp2 = anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 2] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 2]
                    tmp_norm0 = sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 
                    tmp_muscle_len += tmp_norm0
                    res[(anchor_idx + tmp_idx) * 3 + 0] = tmp0 / tmp_norm0
                    res[(anchor_idx + tmp_idx) * 3 + 1] = tmp1 / tmp_norm0
                    res[(anchor_idx + tmp_idx) * 3 + 2] = tmp2 / tmp_norm0

                elif tmp_idx == hold_num_buf[muscle_idx] - 1:
                    tmp0 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 0] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 0]
                    tmp1 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 1] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 1]
                    tmp2 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 2] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 2]
                    tmp_norm0 = sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 
                    ...
                    tmp_norm0 = sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 
                    tmp_muscle_len += tmp_norm0
                    tmp3 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 0] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 0]
                    tmp4 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 1] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 1]
                    tmp5 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 2] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 2]
                    tmp_norm1 = sqrt(tmp3 * tmp3 + tmp4 * tmp4 + tmp5 * tmp5) 
                    res[(anchor_idx + tmp_idx) * 3 + 0] = tmp0 / tmp_norm0 + tmp3 / tmp_norm1
                    res[(anchor_idx + tmp_idx) * 3 + 1] = tmp1 / tmp_norm0 + tmp4 / tmp_norm1
                    res[(anchor_idx + tmp_idx) * 3 + 2] = tmp2 / tmp_norm0 + tmp5 / tmp_norm1


                tmp_idx += 1

            tmp_muscle_len /= muscle_origin_length_calculated_buf[muscle_idx]
            tmp_muscle_len -= lt0_buf[muscle_idx]
            tmp_muscle_len /= lm0_buf[muscle_idx]
            tmp_muscle_amplitude = exp(-(tmp_muscle_len - 1.0) * (tmp_muscle_len - 1.0) / gamma) * f0_buf[muscle_idx]

            tmp_idx = 0
            while tmp_idx < hold_num_buf[muscle_idx]:
                res[(anchor_idx + tmp_idx) * 3 + 0] *= tmp_muscle_amplitude
                res[(anchor_idx + tmp_idx) * 3 + 1] *= tmp_muscle_amplitude
                res[(anchor_idx + tmp_idx) * 3 + 2] *= tmp_muscle_amplitude
                tmp_idx += 1

            anchor_idx += hold_num_buf[muscle_idx]
            muscle_idx += 1
            

        return anchor_active_force    
    
    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_just_get_pd_anchor_force_direction(self, 
                                                        np.ndarray[np.float64_t, ndim=1]anchor_pos, 
                                                        np.ndarray[np.int32_t, ndim=1]muscle_hold_anchor_num,
                                                        dReal* anchor_force_direction_ptr):
        cdef np.ndarray[np.float64_t, ndim=1] anchor_global_pos_buf = np.ascontiguousarray(anchor_pos, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim=1] hold_num_buf = np.ascontiguousarray(muscle_hold_anchor_num, dtype=np.int32)
        ...
        cdef dReal tmp_norm0
        cdef dReal tmp_norm1
        cdef dReal tmp_muscle_len_diff
        cdef dReal tmp_muscle_amplitude
        ...
        while muscle_idx < muscle_num :
            ...
            while tmp_idx < hold_num_buf[muscle_idx]:
                if tmp_idx == 0 :
                    ...
                    tmp_norm0 = sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 
                    # tmp_muscle_len += tmp_norm0
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 0] = tmp0 / tmp_norm0
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 1] = tmp1 / tmp_norm0
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 2] = tmp2 / tmp_norm0
                    ...
                    tmp5 = anchor_global_pos_buf[(anchor_idx + tmp_idx - 1) * 3 + 2] - anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 2]
                    tmp_norm1 = sqrt(tmp3 * tmp3 + tmp4 * tmp4 + tmp5 * tmp5) 
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 0] = tmp0 / tmp_norm0 + tmp3 / tmp_norm1
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 1] = tmp1 / tmp_norm0 + tmp4 / tmp_norm1
                    anchor_force_direction_ptr[(anchor_idx + tmp_idx) * 3 + 2] = tmp2 / tmp_norm0 + tmp5 / tmp_norm1
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_pick_bigger_muscle_force(self, 
                                        np.ndarray[np.float64_t, ndim=1] last_max_muscle_force, 
                                        np.ndarray[np.float64_t, ndim=1] instant_muscle_force):
        cdef np.ndarray[np.float64_t, ndim=1] last_max_muscle_force_buf = np.ascontiguousarray(last_max_muscle_force, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] instant_muscle_force_buf = np.ascontiguousarray(instant_muscle_force, dtype=np.float64)
        cdef int muscle_cnt = last_max_muscle_force_buf.size
        cdef np.ndarray[np.float64_t, ndim=1] new_max_muscle_force = np.zeros([muscle_cnt], dtype=np.float64)
        cdef dReal * res = <dReal *> new_max_muscle_force.data
        cdef int idx = 0

        while idx < muscle_cnt:
            if last_max_muscle_force_buf[idx] > instant_muscle_force_buf[idx]:
                res[idx] = last_max_muscle_force_buf[idx]
            else:
                res[idx] = instant_muscle_force_buf[idx]
            idx += 1
        
        return new_max_muscle_force
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle(self, 
                                                dReal target_load, 
                                                dReal sim_hz,
                                                dReal r_para, 
                                                dReal F_para, 
                                                ...
                                                dReal LR_para, 
                                                dReal LD_para,
                                                dReal * fatigue_properties_ptr):
        cdef dReal activated_proportion = fatigue_properties_ptr[0]
        cdef dReal resting_proportion = fatigue_properties_ptr[1]
        cdef dReal fatigued_proportion = fatigue_properties_ptr[2]
        
        cdef dReal next_fatigued_proportion = 0.0
        cdef dReal next_resting_proportion = 0.0
        cdef dReal next_activated_proportion = 0.0
        cdef dReal R_r = R_para
        cdef dReal C_t = 0.0
        
        if activated_proportion >= target_load:
            R_r = r_para * R_para
            C_t = LR_para * (target_load - activated_proportion)
        elif activated_proportion< target_load and resting_proportion > target_load - activated_proportion:
            R_r = R_para
            C_t = LD_para * (target_load - activated_proportion)
        else:
            R_r = R_para
            C_t = LD_para * resting_proportion
        next_activated_proportion = activated_proportion + 1/sim_hz * C_t - 1/sim_hz * F_para * activated_proportion
        next_resting_proportion = resting_proportion - 1/sim_hz * C_t + 1/sim_hz * R_r * fatigued_proportion
        next_fatigued_proportion = fatigued_proportion + 1/sim_hz * F_para * activated_proportion - 1/sim_hz * R_r * fatigued_proportion
        activated_proportion = next_activated_proportion
        resting_proportion = next_resting_proportion
        fatigued_proportion = next_fatigued_proportion

        fatigue_properties_ptr[0] = next_activated_proportion
        fatigue_properties_ptr[1] = next_resting_proportion
        fatigue_properties_ptr[2] = next_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated(self, 
                                                                    dReal sim_hz,
                                                                    dReal r_para, 
                                                                    dReal F_para, 
                                                                    dReal R_para, 
                                                                    ...
                                                                    dReal LD_para,
                                                                    dReal * fatigue_properties_ptr,
                                                                    dReal next_activated_proportion):
        """
        calculate at 120Hz
        do not using 1200Hz like the origin 3cc-fatigue model for simplification
        """
        cdef dReal activated_proportion = fatigue_properties_ptr[0]
        cdef dReal resting_proportion = fatigue_properties_ptr[1]
        cdef dReal fatigued_proportion = fatigue_properties_ptr[2]
        ...
        cdef dReal R_r = R_para
        cdef dReal C_t = 0.0
        cdef dReal tmp_decay_MA
        cdef dReal tmp_target_load

        tmp_decay_MA = (1 - 1/sim_hz * F_para) * activated_proportion

        if next_activated_proportion <= tmp_decay_MA:
            tmp_target_load =  (next_activated_proportion - tmp_decay_MA + 1/sim_hz * LR_para *activated_proportion) / (1/sim_hz * LR_para)
            if tmp_target_load> 1:
                tmp_target_load = 1
            if tmp_target_load < 0:
                tmp_target_load = 0
            R_r = r_para * R_para
            C_t = LR_para * (tmp_target_load - activated_proportion)
        elif next_activated_proportion > tmp_decay_MA and next_activated_proportion <= tmp_decay_MA + 1/sim_hz * LD_para * resting_proportion:
            tmp_target_load = (next_activated_proportion - tmp_decay_MA + 1/sim_hz * LD_para *activated_proportion) / (1/sim_hz * LD_para)
            if tmp_target_load> 1:
                tmp_target_load = 1
            if tmp_target_load < 0:
                tmp_target_load = 0
            R_r = R_para
            C_t = LD_para * (tmp_target_load - activated_proportion)
        else:
            R_r = R_para
            C_t = LD_para * resting_proportion
        next_resting_proportion = resting_proportion - 1/sim_hz * C_t + 1/sim_hz * R_r * fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_with_input_target_load(self, 
                                                                    dReal sim_hz,
                                                                    dReal r_para, 
                                                                    dReal F_para, 
                                                                    dReal R_para, 
        ...
        if next_activated_proportion <= tmp_decay_MA:
            # input_target_load = (next_activated_proportion - tmp_decay_MA + 1/sim_hz * LR_para *activated_proportion) / (1/sim_hz * LR_para)
            R_r = r_para * R_para
            C_t = LR_para * (input_target_load - activated_proportion)
        elif next_activated_proportion > tmp_decay_MA and next_activated_proportion <= tmp_decay_MA + 1/sim_hz * LD_para * resting_proportion:
            # input_target_load = (next_activated_proportion - tmp_decay_MA + 1/sim_hz * LD_para *activated_proportion) / (1/sim_hz * LD_para)
            R_r = R_para
            C_t = LD_para * (input_target_load - activated_proportion)
        else:
            R_r = R_para
            C_t = LD_para * resting_proportion
        next_resting_proportion = resting_proportion - 1/sim_hz * C_t + 1/sim_hz * R_r * fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load(self, 
                                                                    dReal sim_hz,
                                                                    dReal r_para, 
                                                                    dReal F_para, 
                                                                    dReal R_para, 
        ...
        cdef dReal C_t = 0.0
        cdef dReal tmp_decay_MA
        cdef dReal tmp_target_load = 0.0
        ...
        if next_activated_proportion <= tmp_decay_MA:
            ...
            ## Actually, in this case, the target load is not useful
            ## We set it as the MA + MR
            tmp_target_load = activated_proportion + resting_proportion
            R_r = R_para
            C_t = LD_para * resting_proportion
        next_resting_proportion = resting_proportion - 1/sim_hz * C_t + 1/sim_hz * R_r * fatigued_proportion
        next_fatigued_proportion = fatigued_proportion + 1/sim_hz * F_para * activated_proportion - 1/sim_hz * R_r * fatigued_proportion
        ...
        calculated_target_load[0] = tmp_target_load
        fatigue_properties_ptr[0] = next_activated_proportion
        fatigue_properties_ptr[1] = next_resting_proportion
        fatigue_properties_ptr[2] = next_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load_1200Hz_calculated(self, 
                                                                    dReal sim_hz,
                                                                    dReal r_para, 
                                                                    dReal F_para, 
                                                                    dReal R_para, 
        ...
        cdef dReal tmp_decay_MA
        cdef dReal tmp_target_load = 0.0
        cdef int calc_times = 10
        cdef dReal calc_hz = 1200.0
        ...
        for i in range(calc_times):
            if next_activated_proportion <= tmp_decay_MA:
                ...
                C_t = LD_para * resting_proportion
            next_activated_proportion = activated_proportion + 1/calc_hz * C_t - 1/calc_hz * F_para * activated_proportion
            next_resting_proportion = resting_proportion - 1/calc_hz * C_t + 1/calc_hz * R_r * fatigued_proportion
            next_fatigued_proportion = fatigued_proportion + 1/calc_hz * F_para * activated_proportion - 1/calc_hz * R_r * fatigued_proportion
            activated_proportion = next_activated_proportion
            resting_proportion = next_resting_proportion
            fatigued_proportion = next_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load_12000Hz_calculated(self, 
                                                                    dReal sim_hz,
                                                                    dReal r_para, 
                                                                    dReal F_para, 
                                                                    dReal R_para, 
        ...
        cdef dReal tmp_decay_MA
        cdef dReal tmp_target_load = 0.0
        cdef int calc_times = 100
        cdef dReal calc_hz = 12000.0
        cdef dReal last_resting = resting_proportion
        cdef dReal calc_activate_proportion = next_activated_proportion
        ...
        for i in range(calc_times):
            if next_activated_proportion <= tmp_decay_MA:
                C_t = LR_para * (tmp_target_load - activated_proportion)
            elif next_activated_proportion > tmp_decay_MA and next_activated_proportion <= tmp_decay_MA + 1/sim_hz * LD_para * last_resting:
                C_t = LD_para * (tmp_target_load - activated_proportion)
            else:
                C_t = LD_para * resting_proportion
            next_activated_proportion = activated_proportion + 1/calc_hz * C_t - 1/calc_hz * F_para * activated_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_fatigue_step(self, 
                            np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                            np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                            np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                            np.ndarray[np.float64_t, ndim=1] muscle_next_activation, 
                            dReal sim_hz,
                            dReal r_para, 
                            dReal F_para, 
                            dReal R_para, 
                            dReal LR_para, 
                            dReal LD_para):
        cdef np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion_buf = np.ascontiguousarray(muscle_fatigued_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion_buf = np.ascontiguousarray(muscle_resting_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion_buf = np.ascontiguousarray(muscle_activated_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_next_activation_buf = np.ascontiguousarray(muscle_next_activation, dtype=np.float64)
        
        cdef np.ndarray[np.float64_t, ndim=1] fatigue_properties = np.zeros([3], dtype=np.float64)
        cdef dReal * res =  <dReal *> fatigue_properties.data
        cdef int muscles_num = muscle_fatigued_proportion_buf.size
        cdef np.ndarray[np.float64_t, ndim=1] muscle_target_load = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* muscle_target_load_ptr = <dReal*> muscle_target_load.data

        cdef int idx = 0
        cdef np.ndarray[np.float64_t, ndim=1] next_muscle_activated_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* next_muscle_activated_proportion_ptr = <dReal*> next_muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] next_muscle_resting_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* next_muscle_resting_proportion_ptr = <dReal*> next_muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] next_muscle_fatigued_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* next_muscle_fatigued_proportion_ptr = <dReal*> next_muscle_fatigued_proportion.data
        
        cdef dReal right_constraint
        cdef dReal left_constraint
        cdef dReal tmp_target_load
        cdef dReal tmp_new_activated_proportion
        while idx < muscles_num:
            
            tmp_new_activated_proportion = muscle_next_activation_buf[idx]
            

            res[0] = muscle_activated_proportion_buf[idx]
            res[1] = muscle_resting_proportion_buf[idx]
            res[2] = muscle_fatigued_proportion_buf[idx]


            self.only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated(sim_hz, r_para, F_para, R_para, LR_para, LD_para, res, tmp_new_activated_proportion)
            next_muscle_activated_proportion_ptr[idx] = res[0]
            next_muscle_resting_proportion_ptr[idx] = res[1]
            next_muscle_fatigued_proportion_ptr[idx] = res[2]
            
            idx += 1
        
        return next_muscle_activated_proportion, next_muscle_resting_proportion, next_muscle_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_step_using_input_target_load(self, 
                                                    np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                                                    np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                                                    np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                                                    np.ndarray[np.float64_t, ndim=1] muscle_next_activation, 
                                                    np.ndarray[np.float64_t, ndim=1] input_target_load, 
                                                    dReal sim_hz,
                                                    dReal r_para, 
                                                    dReal F_para, 
                                                    dReal R_para, 
        ...
        cdef np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion_buf = np.ascontiguousarray(muscle_activated_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_next_activation_buf = np.ascontiguousarray(muscle_next_activation, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] input_target_load_buf = np.ascontiguousarray(input_target_load, dtype=np.float64)
        ...
        while idx < muscles_num:
            ...
            self.only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_with_input_target_load(sim_hz, r_para, F_para, R_para, LR_para, LD_para, res, tmp_new_activated_proportion, input_target_load_buf[idx])
            next_muscle_activated_proportion_ptr[idx] = res[0]
            next_muscle_resting_proportion_ptr[idx] = res[1]
            next_muscle_fatigued_proportion_ptr[idx] = res[2]
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_step_using_calc_target_load_with_outout_target_load(self, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_next_activation, 
        ...
        cdef dReal * res =  <dReal *> fatigue_properties.data
        cdef int muscles_num = muscle_fatigued_proportion_buf.size
        cdef np.ndarray[np.float64_t, ndim=1] target_load_properties = np.zeros([1], dtype=np.float64)
        cdef dReal * res1 =  <dReal *> target_load_properties.data
        cdef np.ndarray[np.float64_t, ndim=1] muscle_target_load = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* muscle_target_load_ptr = <dReal*> muscle_target_load.data
        ...
        cdef np.ndarray[np.float64_t, ndim=1] target_load = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* target_load_ptr = <dReal*> target_load.data
        ...
        while idx < muscles_num:
            ...
            self.only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load(sim_hz, r_para, F_para, R_para, LR_para, LD_para, res1, res, tmp_new_activated_proportion)
            next_muscle_activated_proportion_ptr[idx] = res[0]
            next_muscle_resting_proportion_ptr[idx] = res[1]
            next_muscle_fatigued_proportion_ptr[idx] = res[2]
            target_load_ptr[idx] = res1[0]
            
            idx += 1
        
        return next_muscle_activated_proportion, next_muscle_resting_proportion, next_muscle_fatigued_proportion, target_load

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_step_using_calc_target_load_with_outout_target_load_1200Hz_calculated(self, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_next_activation, 
        ...
        while idx < muscles_num:
            ...
            self.only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load_1200Hz_calculated(sim_hz, r_para, F_para, R_para, LR_para, LD_para, res1, res, tmp_new_activated_proportion)
            next_muscle_activated_proportion_ptr[idx] = res[0]
            next_muscle_resting_proportion_ptr[idx] = res[1]
            next_muscle_fatigued_proportion_ptr[idx] = res[2]
            target_load_ptr[idx] = res1[0]
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_step_using_calc_target_load_with_outout_target_load_12000Hz_calculated(self, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_next_activation, 
        ...
        while idx < muscles_num:
            ...
            self.only_cython_3cc_fatigue_one_piece_muscle_with_MA_next_calculated_output_target_load_12000Hz_calculated(sim_hz, r_para, F_para, R_para, LR_para, LD_para, res1, res, tmp_new_activated_proportion)
            next_muscle_activated_proportion_ptr[idx] = res[0]
            next_muscle_resting_proportion_ptr[idx] = res[1]
            next_muscle_fatigued_proportion_ptr[idx] = res[2]
            target_load_ptr[idx] = res1[0]
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_part_fatigue_properties(self, 
                                            np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion,
                                            np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion, 
                                            np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion, 
                                            np.ndarray[np.float64_t, ndim=1] muscle_part_weight, 
                                            np.ndarray[np.int32_t, ndim=1] muscle_part_idx):
        cdef np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion_buf = np.ascontiguousarray(muscle_fatigued_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion_buf = np.ascontiguousarray(muscle_resting_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion_buf = np.ascontiguousarray(muscle_activated_proportion, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_part_weight_buf = np.ascontiguousarray(muscle_part_weight, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim=1] muscle_part_idx_buf = np.ascontiguousarray(muscle_part_idx, dtype=np.int32)
        cdef int muscles_num = muscle_fatigued_proportion_buf.size
        cdef int idx = 0
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_activated_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_activated_proportion_ptr = <dReal*> part_muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_resting_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_resting_proportion_ptr = <dReal*> part_muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_fatigued_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_fatigued_proportion_ptr = <dReal*> part_muscle_fatigued_proportion.data
        while idx < muscles_num:
            part_muscle_activated_proportion_ptr[muscle_part_idx[idx]] += muscle_activated_proportion_buf[idx] * muscle_part_weight_buf[idx]
            part_muscle_resting_proportion_ptr[muscle_part_idx[idx]] += muscle_resting_proportion_buf[idx] * muscle_part_weight_buf[idx]
            part_muscle_fatigued_proportion_ptr[muscle_part_idx[idx]] += muscle_fatigued_proportion_buf[idx] * muscle_part_weight_buf[idx]
            
            idx += 1
        return part_muscle_activated_proportion, part_muscle_resting_proportion, part_muscle_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_calc_integral_3cc_model(self, 
                                        dReal force_load,
                                        dReal time_horizon,  
                                        dReal force_load_horizon, 
                                        dReal sim_hz,
                                        dReal r_para, 
                                        dReal F_para, 
                                        dReal R_para, 
        ...
        cdef np.ndarray[np.float64_t, ndim=1] fatigue_properties = np.zeros([3], dtype=np.float64)
        cdef dReal * res =  <dReal *> fatigue_properties.data
        cdef int times = <int>(sim_hz / (1 / time_horizon))
        cdef int force_load_times = <int>(sim_hz / (1 / force_load_horizon))
        cdef dReal activated_proportion = 0.0
        cdef dReal resting_proportion = 1.0
        cdef dReal fatigued_proportion = 0.0
        cdef dReal next_activated_proportion = 0.0
        cdef dReal next_resting_proportion = 1.0
        cdef dReal next_fatigued_proportion = 0.0
        cdef int time_idx = 0
        cdef np.ndarray[np.float64_t, ndim=1] integral_muscle_activated_proportion = np.zeros([times], dtype=np.float64)
        cdef dReal* integral_muscle_activated_proportion_ptr = <dReal*> integral_muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] integral_muscle_resting_proportion = np.zeros([times], dtype=np.float64)
        cdef dReal* integral_muscle_resting_proportion_ptr = <dReal*> integral_muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] integral_muscle_fatigued_proportion = np.zeros([times], dtype=np.float64)
        cdef dReal* integral_muscle_fatigued_proportion_ptr = <dReal*> integral_muscle_fatigued_proportion.data
        res[0] = activated_proportion
        res[1] = resting_proportion
        res[2] = fatigued_proportion
        while time_idx < times:
            integral_muscle_activated_proportion_ptr[time_idx] = res[0]
            integral_muscle_resting_proportion_ptr[time_idx] = res[1]
            integral_muscle_fatigued_proportion_ptr[time_idx] = res[2]
            if time_idx < force_load_times:
                self.only_cython_3cc_fatigue_one_piece_muscle(force_load, sim_hz, r_para, F_para, R_para, LR_para, LD_para, res)
            else:
                self.only_cython_3cc_fatigue_one_piece_muscle(0.0, sim_hz, r_para, F_para, R_para, LR_para, LD_para, res)
            
            time_idx += 1
        
        return integral_muscle_activated_proportion, integral_muscle_resting_proportion, integral_muscle_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_integral_part_fatigue_properties(self, 
                                                            np.ndarray[np.int32_t, ndim=1] muscle_part_idx, 
                                                            dReal target_load,
                                                            np.ndarray[np.float64_t, ndim=1] random_time, 
                                                            dReal sim_hz,
                                                            dReal r_para, 
                                                            dReal F_para, 
                                                            dReal R_para, 
                                                            dReal LR_para, 
                                                            dReal LD_para,
                                                            np.ndarray[np.float64_t, ndim=1] integral_3cc_model_activated,
                                                            np.ndarray[np.float64_t, ndim=1] integral_3cc_model_resting, 
                                                            np.ndarray[np.float64_t, ndim=1] integral_3cc_model_fatigued):
        cdef np.ndarray[np.int32_t, ndim=1] muscle_part_idx_buf = np.ascontiguousarray(muscle_part_idx, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] random_time_buf = np.ascontiguousarray(random_time, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] integral_3cc_model_activated_buf = np.ascontiguousarray(integral_3cc_model_activated, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] integral_3cc_model_resting_buf = np.ascontiguousarray(integral_3cc_model_resting, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] integral_3cc_model_fatigued_buf = np.ascontiguousarray(integral_3cc_model_fatigued, dtype=np.float64)
        cdef int idx = 0
        cdef int piece_idx = 0
        cdef int muscles_num = muscle_part_idx_buf.size
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_activated_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_activated_proportion_ptr = <dReal*> part_muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_resting_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_resting_proportion_ptr = <dReal*> part_muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_fatigued_proportion = np.zeros([5], dtype=np.float64)
        cdef dReal* part_muscle_fatigued_proportion_ptr = <dReal*> part_muscle_fatigued_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] muscle_activated_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* muscle_activated_proportion_ptr = <dReal*> muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] muscle_resting_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* muscle_resting_proportion_ptr = <dReal*> muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] muscle_fatigued_proportion = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* muscle_fatigued_proportion_ptr = <dReal*> muscle_fatigued_proportion.data
        
        while idx < 5:
            piece_idx = <int>(random_time[idx] / (1.0 / sim_hz))
            part_muscle_activated_proportion_ptr[idx] = integral_3cc_model_activated_buf[piece_idx]
            part_muscle_resting_proportion_ptr[idx] = integral_3cc_model_resting_buf[piece_idx]
            part_muscle_fatigued_proportion_ptr[idx] = integral_3cc_model_fatigued_buf[piece_idx]
            
            idx += 1

        idx = 0
        while idx < muscles_num:
            muscle_activated_proportion_ptr[idx] = part_muscle_activated_proportion_ptr[muscle_part_idx_buf[idx]]
            muscle_resting_proportion_ptr[idx] = part_muscle_resting_proportion_ptr[muscle_part_idx_buf[idx]]
            muscle_fatigued_proportion_ptr[idx] = part_muscle_fatigued_proportion_ptr[muscle_part_idx_buf[idx]]
            idx += 1

        return part_muscle_activated_proportion, part_muscle_resting_proportion, part_muscle_fatigued_proportion, muscle_activated_proportion, muscle_resting_proportion, muscle_fatigued_proportion
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_3cc_fatigue_integral_part_fatigue_properties_group_randomness(self, 
                                                                            np.ndarray[np.int32_t, ndim=1] muscle_part_idx, 
                                                                            dReal target_load,
                                                                            np.ndarray[np.float64_t, ndim=1] random_time, 
                                                                            dReal sim_hz,
                                                                            ...
                                                                            np.ndarray[np.float64_t, ndim=1] integral_3cc_model_resting, 
                                                                            np.ndarray[np.float64_t, ndim=1] integral_3cc_model_fatigued,
                                                                            np.ndarray[np.int32_t, ndim=1] which_group,
                                                                            int group_num,
                                                                            np.ndarray[np.float64_t, ndim=1] muscle_part_weight):
        cdef np.ndarray[np.int32_t, ndim=1] muscle_part_idx_buf = np.ascontiguousarray(muscle_part_idx, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] random_time_buf = np.ascontiguousarray(random_time, dtype=np.float64)
        ...
        cdef np.ndarray[np.float64_t, ndim=1] integral_3cc_model_resting_buf = np.ascontiguousarray(integral_3cc_model_resting, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] integral_3cc_model_fatigued_buf = np.ascontiguousarray(integral_3cc_model_fatigued, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim=1] which_group_buf = np.ascontiguousarray(which_group, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_part_weight_buf = np.ascontiguousarray(muscle_part_weight, dtype=np.float64)
        cdef int idx = 0
        cdef int piece_idx = 0
        cdef int muscles_num = muscle_part_idx_buf.size

        cdef np.ndarray[np.float64_t, ndim=1] group_muscle_activated_proportion = np.zeros([group_num], dtype=np.float64)
        cdef dReal* group_muscle_activated_proportion_ptr = <dReal*> group_muscle_activated_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] group_muscle_resting_proportion = np.zeros([group_num], dtype=np.float64)
        cdef dReal* group_muscle_resting_proportion_ptr = <dReal*> group_muscle_resting_proportion.data
        cdef np.ndarray[np.float64_t, ndim=1] group_muscle_fatigued_proportion = np.zeros([group_num], dtype=np.float64)
        cdef dReal* group_muscle_fatigued_proportion_ptr = <dReal*> group_muscle_fatigued_proportion.data
        ...
        while idx < group_num:
            piece_idx = <int>(random_time[idx] / (1.0 / sim_hz))
            group_muscle_activated_proportion_ptr[idx] = integral_3cc_model_activated_buf[piece_idx]
            group_muscle_resting_proportion_ptr[idx] = integral_3cc_model_resting_buf[piece_idx]
            group_muscle_fatigued_proportion_ptr[idx] = integral_3cc_model_fatigued_buf[piece_idx]
            
            idx += 1

        idx = 0
        while idx < muscles_num:
            muscle_activated_proportion_ptr[idx] = group_muscle_activated_proportion_ptr[which_group_buf[idx]]
            muscle_resting_proportion_ptr[idx] = group_muscle_resting_proportion_ptr[which_group_buf[idx]]
            muscle_fatigued_proportion_ptr[idx] = group_muscle_fatigued_proportion_ptr[which_group_buf[idx]]
            idx += 1

        idx = 0
        while idx < muscles_num:
            part_muscle_activated_proportion_ptr[muscle_part_idx[idx]] += muscle_activated_proportion_ptr[idx] * muscle_part_weight_buf[idx]
            part_muscle_resting_proportion_ptr[muscle_part_idx[idx]] += muscle_resting_proportion_ptr[idx] * muscle_part_weight_buf[idx]
            part_muscle_fatigued_proportion_ptr[muscle_part_idx[idx]] += muscle_fatigued_proportion_ptr[idx] * muscle_part_weight_buf[idx]
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_calc_part_fatigue_weight(self, 
                                        np.ndarray[np.float64_t, ndim=1] muscle_max_force, 
                                        np.ndarray[np.int32_t, ndim=1] muscle_part_idx):
        cdef np.ndarray[np.float64_t, ndim=1] muscle_max_force_buf = np.ascontiguousarray(muscle_max_force, dtype=np.float64)
        cdef np.ndarray[np.int32_t, ndim=1] muscle_part_idx_buf = np.ascontiguousarray(muscle_part_idx, dtype=np.int32)
        cdef int muscles_num = muscle_max_force_buf.size
        cdef int idx = 0
        cdef np.ndarray[np.float64_t, ndim=1] part_muscle_fatigue_weight = np.zeros([muscles_num], dtype=np.float64)
        cdef dReal* res = <dReal*> part_muscle_fatigue_weight.data
        cdef np.ndarray[np.float64_t, ndim=1] part_sum_force = np.zeros([5], dtype=np.float64)
        cdef dReal* part_sum_force_ptr = <dReal*> part_sum_force.data
        while idx < muscles_num:
            part_sum_force_ptr[muscle_part_idx_buf[idx]] += muscle_max_force_buf[idx]

            idx += 1
        idx = 0
        while idx < muscles_num:
            res[idx] = muscle_max_force_buf[idx] / part_sum_force_ptr[muscle_part_idx_buf[idx]]

            idx += 1
        
        return part_muscle_fatigue_weight
    ...
    def cython_calc_part_fatigue_weight_avg(self, 
        ...
        cdef dReal* part_sum_force_ptr = <dReal*> part_sum_force.data
        while idx < muscles_num:
            part_sum_force_ptr[muscle_part_idx_buf[idx]] += 1.0

            idx += 1
        idx = 0
        while idx < muscles_num:
            res[idx] = 1.0 / part_sum_force_ptr[muscle_part_idx_buf[idx]]
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_apply_pd_muscle_force(self, 
                                    dReal gamma, 
                                    dReal k_pe, 
                                    dReal e_mo, 
                                    np.ndarray[np.float64_t, ndim=1]muscle_properties1, 
                                    np.ndarray[np.float64_t, ndim=1]muscle_properties2, 
                                    np.ndarray[np.float64_t, ndim=1]muscle_origin_length_calculated,
                                    np.ndarray[np.float64_t, ndim=1]pose_aware_f0_max, 
                                    np.ndarray[np.float64_t, ndim=1]anchor_pos, 
                                    np.ndarray[np.float64_t, ndim=1]sim_muscle_len, 
                                    np.ndarray[np.float64_t, ndim=1]ref_muscle_len, 
                                    np.ndarray[np.float64_t, ndim=1]last_frame_sim_muscle_len, 
                                    np.ndarray[np.float64_t, ndim=1]kp_coff, 
                                    np.ndarray[np.float64_t, ndim=1]kd_coff, 
                                    dReal fps,
                                    np.ndarray[np.int32_t, ndim=1]muscle_hold_anchor_num,
                                    np.ndarray[np.float64_t, ndim=1] muscle_last_MF,
                                    np.ndarray[np.float64_t, ndim=1] muscle_last_MA,
                                    np.ndarray[np.float64_t, ndim=1] muscle_last_MR,
                                    dReal LD_para,
                                    dReal LR_para,
                                    dReal F_para):
        cdef np.ndarray[np.int32_t, ndim=1] hold_num_buf = np.ascontiguousarray(muscle_hold_anchor_num, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] pose_aware_f0_max_buf = np.ascontiguousarray(pose_aware_f0_max, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] sim_muscle_len_buf = np.ascontiguousarray(sim_muscle_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] ref_muscle_len_buf = np.ascontiguousarray(ref_muscle_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] last_frame_sim_muscle_len_buf = np.ascontiguousarray(last_frame_sim_muscle_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] kp_coff_buf = np.ascontiguousarray(kp_coff, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] kd_coff_buf = np.ascontiguousarray(kd_coff, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MF_buf = np.ascontiguousarray(muscle_last_MF, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MA_buf = np.ascontiguousarray(muscle_last_MA, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MR_buf = np.ascontiguousarray(muscle_last_MR, dtype=np.float64)

        cdef np.ndarray[np.float64_t, ndim=1] muscle_origin_length_calculated_buf = np.ascontiguousarray(muscle_origin_length_calculated, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lm0_buf = np.ascontiguousarray(muscle_properties1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lt0_buf = np.ascontiguousarray(muscle_properties2, dtype=np.float64)

        cdef int anchor_num = anchor_pos.size // 3
        cdef int muscle_num = hold_num_buf.size

        cdef np.ndarray[np.float64_t, ndim = 1] anchor_force = np.zeros(anchor_num * 3, dtype=np.float64)
        cdef dReal * res = <dReal *> anchor_force.data
        self.only_cython_just_get_pd_anchor_force_direction(anchor_pos, muscle_hold_anchor_num, res)
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_amplitude = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * muscle_amplitude_ptr = <dReal *> muscle_amplitude.data
        # for debug
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_passive_amplitude = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * muscle_passive_amplitude_ptr = <dReal *> muscle_passive_amplitude.data
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_unclip_amplitude = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * muscle_unclip_amplitude_ptr = <dReal *> muscle_unclip_amplitude.data
        ...
        cdef dReal tmp_muscle_len
        cdef dReal tmp_muscle_len_diff
        cdef dReal tmp_muscle_len_dot
        cdef dReal tmp_muscle_len_dot_norm
        cdef dReal tmp_last_frame_tmp_muscle_len
        cdef dReal tmp_muscle_amplitude
        cdef dReal tmp_muscle_hill_type_ratio_right
        cdef dReal tmp_muscle_hill_type_ratio_left
        cdef dReal tmp_muscle_velo_amplitude
        cdef dReal tmp_muscle_active_amplitude
        cdef dReal tmp_muscle_passive_amplitude

        cdef dReal right_constraint
        cdef dReal left_constraint

        while muscle_idx < muscle_num :
            tmp_muscle_len = sim_muscle_len_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len = last_frame_sim_muscle_len_buf[muscle_idx]
            tmp_muscle_len /= muscle_origin_length_calculated_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len /= muscle_origin_length_calculated_buf[muscle_idx]
            tmp_muscle_len -= lt0_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len -= lt0_buf[muscle_idx]
            tmp_muscle_len /= lm0_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len /= lm0_buf[muscle_idx]
            tmp_muscle_active_amplitude = exp(-(tmp_muscle_len - 1.0) * (tmp_muscle_len - 1.0) / gamma) 
            if (tmp_muscle_len) >= 1.0:   
                tmp_muscle_passive_amplitude = ((exp(k_pe * (tmp_muscle_len - 1.0) / e_mo) - 1.0) / (exp(k_pe) - 1.0)) 
                if tmp_muscle_passive_amplitude >= 2.0:
                    tmp_muscle_passive_amplitude = 2.0  
            else:
                tmp_muscle_passive_amplitude = 0.0

            tmp_muscle_len_diff = sim_muscle_len_buf[muscle_idx] - ref_muscle_len_buf[muscle_idx]
            
            tmp_muscle_len_dot = (sim_muscle_len_buf[muscle_idx] - last_frame_sim_muscle_len_buf[muscle_idx]) / fps
            tmp_muscle_len_dot_norm = (tmp_muscle_len - tmp_last_frame_tmp_muscle_len) / fps

            if tmp_muscle_len_dot_norm > 0.0:
                tmp_muscle_velo_amplitude = 1.5 + 0.5 * (-10 + tmp_muscle_len_dot_norm) / (37.8*tmp_muscle_len_dot_norm+10.0)
            else:
                tmp_muscle_velo_amplitude = (-10 - tmp_muscle_len_dot_norm) / (-10.0+tmp_muscle_len_dot_norm*5.0)

            
            left_constraint = (1 - 1/fps * F_para) * muscle_last_MA_buf[muscle_idx] - 1/fps*LR_para*muscle_last_MA_buf[muscle_idx] 
            if left_constraint < 0.0:
                left_constraint = 0.0 
            right_constraint = (1 - 1/fps * F_para) * muscle_last_MA_buf[muscle_idx] + 1/fps*LD_para*muscle_last_MR_buf[muscle_idx]

            if right_constraint > 1.0:
                right_constraint = 1.0
            
            tmp_muscle_hill_type_ratio_right = right_constraint * tmp_muscle_active_amplitude * tmp_muscle_velo_amplitude+ tmp_muscle_passive_amplitude
            tmp_muscle_hill_type_ratio_left = left_constraint * tmp_muscle_active_amplitude * tmp_muscle_velo_amplitude+ tmp_muscle_passive_amplitude
            
            
            tmp_muscle_amplitude =  kp_coff_buf[muscle_idx] * tmp_muscle_len_diff + kd_coff_buf[muscle_idx] * tmp_muscle_len_dot

            muscle_unclip_amplitude_ptr[muscle_idx] = tmp_muscle_amplitude

            if tmp_muscle_amplitude < 0.0:
                tmp_muscle_amplitude = 0.0

            if tmp_muscle_amplitude > pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_hill_type_ratio_right:
                muscle_amplitude_ptr[muscle_idx] = pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_hill_type_ratio_right
            elif tmp_muscle_amplitude < pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_hill_type_ratio_left:
                muscle_amplitude_ptr[muscle_idx] = pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_hill_type_ratio_left
            else:
                muscle_amplitude_ptr[muscle_idx] = tmp_muscle_amplitude
            
            muscle_passive_amplitude_ptr[muscle_idx] = pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_passive_amplitude

            tmp_idx = 0
            while tmp_idx < hold_num_buf[muscle_idx]:
                res[(anchor_idx + tmp_idx) * 3 + 0] *= muscle_amplitude_ptr[muscle_idx]
                res[(anchor_idx + tmp_idx) * 3 + 1] *= muscle_amplitude_ptr[muscle_idx]
                res[(anchor_idx + tmp_idx) * 3 + 2] *= muscle_amplitude_ptr[muscle_idx]
                tmp_idx += 1
        ...
        return anchor_force, muscle_amplitude, muscle_passive_amplitude, muscle_unclip_amplitude
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_calc_hill_type_activation(self, 
                                            dReal gamma, 
                                            dReal k_pe, 
                                            dReal e_mo, 
                                            np.ndarray[np.float64_t, ndim=1]muscle_properties1, 
                                            ...
                                            np.ndarray[np.float64_t, ndim=1]pose_aware_f0_max, 
                                            dReal fps,
                                            np.ndarray[np.float64_t, ndim=1]muscle_force,
                                            np.ndarray[np.float64_t, ndim=1]muscle_sim_len,
                                            np.ndarray[np.float64_t, ndim=1]last_frame_muscle_sim_len,
                                            np.ndarray[np.float64_t, ndim=1] muscle_last_MF,
                                            np.ndarray[np.float64_t, ndim=1] muscle_last_MA,
                                            np.ndarray[np.float64_t, ndim=1] muscle_last_MR,
                                            dReal LD_para,
        ...
        cdef np.ndarray[np.float64_t, ndim=1] lm0_buf = np.ascontiguousarray(muscle_properties1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lt0_buf = np.ascontiguousarray(muscle_properties2, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_force_buf = np.ascontiguousarray(muscle_force, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_sim_len_buf = np.ascontiguousarray(muscle_sim_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] last_frame_muscle_sim_len_buf = np.ascontiguousarray(last_frame_muscle_sim_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MF_buf = np.ascontiguousarray(muscle_last_MF, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MA_buf = np.ascontiguousarray(muscle_last_MA, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_last_MR_buf = np.ascontiguousarray(muscle_last_MR, dtype=np.float64)


        cdef int muscle_num = muscle_origin_length_calculated_buf.size

        cdef np.ndarray[np.float64_t, ndim = 1] muscle_activation = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * muscle_activation_ptr = <dReal *> muscle_activation.data
        cdef np.ndarray[np.float64_t, ndim = 1] un_clip_muscle_activation = np.zeros(muscle_num, dtype=np.float64)
        cdef dReal * un_clip_muscle_activation_ptr = <dReal *> un_clip_muscle_activation.data

        cdef int muscle_idx = 0
        cdef int tmp_idx = 0

        cdef dReal tmp_muscle_len
        cdef dReal tmp_muscle_activation
        cdef dReal tmp_muscle_amplitude
        cdef dReal tmp_muscle_len_dot_norm
        cdef dReal tmp_last_frame_tmp_muscle_len
        cdef dReal tmp_muscle_active_amplitude
        ...
        while muscle_idx < muscle_num :
            tmp_muscle_len = muscle_sim_len_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len = last_frame_muscle_sim_len_buf[muscle_idx]
            tmp_muscle_len /= muscle_origin_length_calculated_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len /= muscle_origin_length_calculated_buf[muscle_idx]
            tmp_muscle_len -= lt0_buf[muscle_idx]
            tmp_last_frame_tmp_muscle_len -= lt0_buf[muscle_idx]
            ...
            tmp_muscle_amplitude = muscle_force_buf[muscle_idx] -  tmp_muscle_passive_amplitude * pose_aware_f0_max_buf[muscle_idx]
            ...
            tmp_muscle_activation = tmp_muscle_amplitude / (tmp_muscle_active_amplitude * pose_aware_f0_max_buf[muscle_idx] * tmp_muscle_velo_amplitude)
            
            un_clip_muscle_activation_ptr[muscle_idx] = tmp_muscle_activation

            if tmp_muscle_activation > right_constraint :
                tmp_muscle_activation = right_constraint
            if tmp_muscle_activation < left_constraint :
                tmp_muscle_activation = left_constraint
            
            muscle_activation_ptr[muscle_idx] = tmp_muscle_activation
    ...
    def cython_build_coact_matrix(self, 
                                ...
                                np.ndarray[np.float64_t, ndim=1]pose_aware_f0_max):
        """
        using f0 as co-ativation weight
        """
        cdef np.ndarray[np.int32_t, ndim=1] which_group_buf = np.ascontiguousarray(which_group, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim=1] pose_aware_f0_max_buf = np.ascontiguousarray(pose_aware_f0_max, dtype=np.float64)
        
        cdef int muscle_num = which_group.size 
        cdef np.ndarray[np.float64_t, ndim = 1] coact_matrix = np.zeros(muscle_num*muscle_num, dtype=np.float64)
        cdef dReal * res = <dReal *> coact_matrix.data
        cdef np.ndarray[np.float64_t, ndim = 1] group_f0_sum = np.zeros(group_num, dtype=np.float64)
        cdef dReal * group_f0_sum_ptr = <dReal *> group_f0_sum.data
        
        cdef int idx = 0
        cdef int jdx = 0
        while idx < muscle_num :
            group_f0_sum_ptr[which_group_buf[idx]] += pose_aware_f0_max_buf[idx]
            idx += 1
        
        idx = 0
        while idx < muscle_num:
            jdx = 0
            while jdx < muscle_num:
                if which_group_buf[idx] == which_group_buf[jdx]:
                    res[idx * muscle_num + jdx] = pose_aware_f0_max_buf[jdx] /group_f0_sum_ptr[which_group_buf[idx]]
                jdx += 1
            idx += 1
    ...
    def cython_build_coact_matrix_avg(self, 
        ...
        cdef np.ndarray[np.float64_t, ndim = 1] coact_matrix = np.zeros(muscle_num*muscle_num, dtype=np.float64)
        cdef dReal * res = <dReal *> coact_matrix.data
        cdef np.ndarray[np.float64_t, ndim = 1] group_sum = np.zeros(group_num, dtype=np.float64)
        cdef dReal * group_sum_ptr = <dReal *> group_sum.data
        
        cdef int idx = 0
        cdef int jdx = 0
        while idx < muscle_num :
            group_sum_ptr[which_group_buf[idx]] += 1.0
            idx += 1
        ...
        while idx < muscle_num:
            ...
            while jdx < muscle_num:
                if which_group_buf[idx] == which_group_buf[jdx]:
                    res[idx * muscle_num + jdx] = 1.0 /group_sum_ptr[which_group_buf[idx]]
                jdx += 1
            idx += 1
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_get_rot_mat_rel_and_p(self, 
                                          np.ndarray[np.uint64_t, ndim=1]bd_bid,
                                          np.ndarray[np.int32_t, ndim=1]which_parent,
                                          dReal * relative_rotation_matrix,
                                          dReal * parent_rotation_matrix):
        """
        This function get Local Rotation Matrix
        Functionality Just Like cython_just_get_rot_mat_rel
        BUT, Can be Accessed in cython ONLY!
        Serve as a Medium Function
        Change Value: relative_rotation_matrix SHAPE: [body_count * 12]
        """
        cdef np.ndarray[np.uint64_t, ndim=1] bd_bid_buf = np.ascontiguousarray(bd_bid, dtype=np.uint64)
        cdef np.ndarray[np.int32_t, ndim=1] which_parent_buf = np.ascontiguousarray(which_parent, dtype=np.int32)
        cdef int body_count = bd_bid_buf.size

        cdef np.ndarray[np.float64_t, ndim=1] body_rot_together = np.zeros([body_count * 9], dtype=np.float64)
        cdef dReal * gather_bd_rot = <dReal *> body_rot_together.data

        cdef dReal * rot = <dReal *> dBodyGetRotation(<dBodyID> bd_bid_buf[0])

        cdef np.ndarray[np.float64_t, ndim=1] tmp_rot_mat1 = np.zeros([9], dtype=np.float64)
        cdef dReal * tmp_rot_mat_ptr1 = <dReal *> tmp_rot_mat1.data
        cdef np.ndarray[np.float64_t, ndim=1] tmp_rot_mat2 = np.zeros([9], dtype=np.float64)
        cdef dReal * tmp_rot_mat_ptr2 = <dReal *> tmp_rot_mat2.data
        cdef np.ndarray[np.float64_t, ndim=1] tmp_rot_mat3 = np.zeros([9], dtype=np.float64)
        cdef dReal * tmp_rot_mat_ptr3 = <dReal *> tmp_rot_mat3.data
        cdef dMatrix3 tmp_mat1

        cdef int idx = 1

        self.dMatrix3_change_from_ode_mat_format(rot, gather_bd_rot) # 12 9
        self.I3_dMatrix3_copy_to_dMatrix3_format(parent_rotation_matrix) # 12
        self.dMatrix3_copy_to_dMatrix3_format(rot, relative_rotation_matrix) # 12 12

        while idx < body_count:
            rot = <dReal*>dBodyGetRotation(<dBodyID> bd_bid_buf[idx])  
            self.dMatrix3_change_from_ode_mat_format(rot, <dReal *>(gather_bd_rot + 9 * idx))
            self.ode_mat_copy_to_dMatrix3_format(<dReal *>(gather_bd_rot + 9 * which_parent_buf[idx]), <dReal *>(parent_rotation_matrix + 12 * idx))

            tmp_mat1 = <dMatrix3> (parent_rotation_matrix + 12 * idx)
            self.dMatrix3_change_from_ode_mat_format(tmp_mat1, tmp_rot_mat_ptr1)
            tmp_rot_mat_ptr2 = <dReal *> (gather_bd_rot + 9 * idx)
            self.cython_mat1_T_matmul_mat2(tmp_rot_mat_ptr3, tmp_rot_mat_ptr1, tmp_rot_mat_ptr2)
            self.ode_mat_copy_to_dMatrix3_format(tmp_rot_mat_ptr3, <dReal *>(relative_rotation_matrix + 12 * idx))
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def get_muscle_length_by_anchor_pos(self, 
                                        np.ndarray[np.int32_t, ndim=1]muscle_hold_num, 
                                        np.ndarray[np.float64_t, ndim=1]anchor_global_pos, 
                                        np.ndarray[np.float64_t, ndim=1]muscle_ori_len, 
                                        np.ndarray[np.float64_t, ndim=1]tendon_len):
        cdef np.ndarray[np.int32_t, ndim = 1] muscle_hold_num_buf = np.ascontiguousarray(muscle_hold_num, dtype=np.int32)
        cdef np.ndarray[np.float64_t, ndim = 1] anchor_global_pos_buf = np.ascontiguousarray(anchor_global_pos, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_ori_len_buf = np.ascontiguousarray(muscle_ori_len, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim = 1] tendon_len_buf = np.ascontiguousarray(tendon_len, dtype=np.float64)

        cdef int muscle_size = muscle_hold_num_buf.size
        cdef int anchor_size = anchor_global_pos_buf.size // 3
        ...
        while muscle_idx < muscle_size :
            tmp_idx = 0
            while tmp_idx < muscle_hold_num_buf[muscle_idx] - 1:
                tmp0 = anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 0] - anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 0]
                tmp1 = anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 1] - anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 1]
                tmp2 = anchor_global_pos_buf[(anchor_idx + tmp_idx) * 3 + 2] - anchor_global_pos_buf[(anchor_idx + tmp_idx + 1) * 3 + 2]
                res[muscle_idx] += sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2) 

                tmp_idx += 1

            res[muscle_idx] /= muscle_ori_len_buf[muscle_idx]
            res[muscle_idx] -= tendon_len_buf[muscle_idx]
            anchor_idx += muscle_hold_num_buf[muscle_idx]
            muscle_idx += 1
    ...
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_get_F_A(self, 
                       dReal gamma, 
                       np.ndarray[np.float64_t, ndim=1]muscle_properties0, 
                       np.ndarray[np.float64_t, ndim=1]muscle_properties1,
                       np.ndarray[np.float64_t, ndim=1]muscle_length):
        cdef np.ndarray[np.float64_t, ndim=1] f0_buf = np.ascontiguousarray(muscle_properties0, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] lm0_buf = np.ascontiguousarray(muscle_properties1, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] muscle_len_buf = np.ascontiguousarray(muscle_length, dtype=np.float64)

        cdef int muscle_cnt = f0_buf.size

        cdef np.ndarray[np.float64_t, ndim=1] f_a_muscle = np.zeros([muscle_cnt], dtype=np.float64)
        cdef dReal * res = <dReal *> f_a_muscle.data

        cdef int idx = 0
        while idx < muscle_cnt:
            res[idx] = exp(-((muscle_len_buf[idx] / lm0_buf[idx]) - 1.0) * ((muscle_len_buf[idx] / lm0_buf[idx]) - 1.0) / gamma) * f0_buf[idx]
            idx += 1
    ...
    def cython_get_F_P(self, 
        ...
        cdef np.ndarray[np.float64_t, ndim=1] f_p_muscle = np.zeros([muscle_cnt], dtype=np.float64)
        cdef dReal * res = <dReal *> f_p_muscle.data

        cdef int idx = 0
        while idx < muscle_cnt:
            if (muscle_len_buf[idx]  / lm0_buf[idx]) >= 1.0:
                res[idx] = (exp(k_pe * ((muscle_len_buf[idx] / lm0_buf[idx]) - 1.0) / e_mo) - 1.0) / (exp(k_pe) - 1.0) * f0_buf[idx]
            idx += 1
    ...
    @cython.cdivision(True)            
    @cython.initializedcheck(False)
    def map_muscle_amplitude_to_anchor_amplitude(self, 
                                                 np.ndarray[np.int32_t, ndim = 1] num_of_anchors, 
                                                 np.ndarray[np.float64_t, ndim = 1] muscle_amplitude, 
                                                 int anchor_size):
        cdef np.ndarray[np.int32_t, ndim = 1] num_of_anchors_buf = np.ascontiguousarray(num_of_anchors, dtype=np.uint64)
        cdef np.ndarray[np.float64_t, ndim = 1] muscle_amplitude_buf = np.ascontiguousarray(muscle_amplitude, dtype=np.float64)

        cdef int muscle_cnt = num_of_anchors_buf.size 
        
        cdef np.ndarray[np.float64_t, ndim = 1] anchor_amplitude = np.zeros(anchor_size, dtype=np.float64)
        cdef dReal * res = <dReal *> anchor_amplitude.data
        ...
        while idx < muscle_cnt:
            tmp_idx = 0
            while tmp_idx < num_of_anchors_buf[idx]:
                res[anchor_idx + tmp_idx] = muscle_amplitude_buf[idx]
                tmp_idx += 1
            anchor_idx += num_of_anchors_buf[idx]
            idx += 1

        return anchor_amplitude

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    def cython_get_anchor_force_direction(self, 
                                          np.ndarray[np.float64_t, ndim=1]anchor_pos, 
                                          np.ndarray[np.int32_t, ndim=1]hold_num):
        """
        @return: [anchors_num * 3]
        """
        cdef int anchor_num = anchor_pos.size // 3
        cdef int muscle_num = hold_num.size 

        cdef np.ndarray[np.float64_t, ndim=1] anchor_force_direction = np.zeros([anchor_num * 3], dtype=np.float64)
        cdef dReal * res = <dReal *> anchor_force_direction.data

        self.only_cython_just_get_pd_anchor_force_direction(anchor_pos, hold_num, res)
            
        return anchor_force_direction

    @cython.boundscheck(False)
    @cython.wraparound(False)
    @cython.cdivision(True)           
    @cython.initializedcheck(False)
    cdef only_cython_get_local_target_quats(self,
                                            np.ndarray[np.float64_t, ndim = 1] action_will_do,
                                            np.ndarray[np.float64_t, ndim = 1] target_qs,
                                            dReal * local_target_quats):
        """
        This function cal action_will_do @ target_qs
        BUT, Can be Accessed in cython ONLY!
        Serve as a Medium Function
        @input:
        action_will_do Format : Axis Angle
        target_qs Format : Quats
        
        Change Value: local_target_quats SHAPE: [joint_count * 4]
        """
        cdef int joint_count = action_will_do.size // 3
        cdef int idx = 0
        ## action: axis angles
        cdef np.ndarray[np.float64_t, ndim=1] action_will_do_buf = np.ascontiguousarray(action_will_do, dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] action_will_do_axis_angle = np.zeros((joint_count* 4), dtype=np.float64)
        cdef np.ndarray[np.float64_t, ndim=1] action_will_do_matrix_tmp = np.zeros((9), dtype=np.float64)
        cdef dReal * axis_angle = <dReal *> action_will_do_axis_angle.data
        cdef dReal * matrix0 = <dReal *> action_will_do_matrix_tmp.data
        
        ## target qs : quats
        cdef dReal * target_qs_ptr = <dReal *> target_qs.data
        cdef np.ndarray[np.float64_t, ndim=1] target_qs_matrix_tmp = np.zeros((9), dtype=np.float64)
        cdef dReal * matrix1 = <dReal *> target_qs_matrix_tmp.data

        cdef dReal tmp_0
        cdef dReal tmp_1
        cdef dReal tmp_2
        cdef dReal tmp_norm
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_rot_mat = np.zeros([3 * 4], dtype=np.float64)
        cdef dMatrix3 tmp_rot_ptr = <dMatrix3> tmp_rot_mat.data
        cdef np.ndarray[np.float64_t, ndim = 1] tmp_rot_mat_small = np.zeros([9], dtype=np.float64)
        cdef dReal * tmp_rot_ptr_small = <dReal *> tmp_rot_mat_small.data
        cdef dQuaternion tmp_quat = <dQuaternion> target_qs_ptr
        cdef dQuaternion tmp_quat_adjusted = <dQuaternion> target_qs_ptr
        while idx < joint_count:
            tmp0 = action_will_do_buf[3 * idx + 0]
            tmp1 = action_will_do_buf[3 * idx + 1]
            tmp2 = action_will_do_buf[3 * idx + 2]
            tmp_norm = sqrt(tmp0 * tmp0 + tmp1 * tmp1 + tmp2 * tmp2 )
            axis_angle[4 * idx + 0] = tmp0 / tmp_norm
            axis_angle[4 * idx + 1] = tmp1 / tmp_norm
            axis_angle[4 * idx + 2] = tmp2 / tmp_norm
            axis_angle[4 * idx + 3] = tmp_norm

            dRFromAxisAndAngle(tmp_rot_ptr, axis_angle[4 * idx + 0], axis_angle[4 * idx + 1], axis_angle[4 * idx + 2], axis_angle[4 * idx + 3])
            self.dMatrix3_change_from_ode_mat_format(tmp_rot_ptr, matrix0)
            
            tmp_quat = <dQuaternion> (target_qs_ptr + 4 * idx)
            ## take consideration about the quat order!
            tmp_quat_adjusted[0] = tmp_quat[3]
            tmp_quat_adjusted[1] = tmp_quat[0]
            tmp_quat_adjusted[2] = tmp_quat[1]
            ...
            dRfromQ(tmp_rot_ptr, tmp_quat_adjusted)

            self.dMatrix3_change_from_ode_mat_format(tmp_rot_ptr, matrix1)

            self.cython_mat1_matmul_mat2(tmp_rot_ptr_small, matrix0, matrix1)

            self.ode_mat_copy_to_dMatrix3_format(tmp_rot_ptr_small, tmp_rot_ptr)
            
            dQfromR (tmp_quat, tmp_rot_ptr)
            local_target_quats[4 * idx + 0] = tmp_quat[0]
            local_target_quats[4 * idx + 1] = tmp_quat[1]
            local_target_quats[4 * idx + 2] = tmp_quat[2]
            local_target_quats[4 * idx + 3] = tmp_quat[3]
...
cdef class Body:
    ...
    @property
    def whether_render(self):
        return dBodyGetWhetherRender(self.bid)

    
    @whether_render.setter
    def whether_render(self, int value):
        dBodySetWhetherRender(self.bid, value)
...
cdef class FixedJoint(Joint):
    ...
    #     @type pos: 3-sequence of floats
    #     """
    #     dJointSetFixedAnchor(self.jid, pos[0], pos[1], pos[2])
    ...
    #     cdef dVector3 p
    #     dJointGetFixedAnchor(self.jid, p)
    #     return p[0], p[1], p[2]
    ...
    #     cdef dVector3 p
    #     dJointGetFixedAnchor2(self.jid, p)
    #     return p[0], p[1], p[2]
    ...
    # @cython.wraparound(False)
    # def getAnchor1Raw(self):
    #     cdef const dReal * res = dJointGetFixedAnchor1Raw(self.jid)
    #     return res[0], res[1], res[2]

    # @cython.boundscheck(False)
    # @cython.wraparound(False)
    # def getAnchor2Raw(self):
    #     cdef const dReal * res = dJointGetFixedAnchor2Raw(self.jid)
    #     return res[0], res[1], res[2]

    # @cython.boundscheck(False)
    # @cython.wraparound(False)
    # def getAnchor1RawNumpy(self) -> np.ndarray:
    #     cdef const dReal * p = dJointGetFixedAnchor1Raw(self.jid)
    #     cdef np.ndarray[np.float64_t, ndim = 1] np_buff = np.zeros(3)
    #     cdef dReal * res = <dReal *> np_buff.data
    #     res[0] = p[0]
    #     res[1] = p[1]
    ...
    # @cython.wraparound(False)
    # def getAnchor2RawNumpy(self) -> np.ndarray:
    #     cdef const dReal * p = dJointGetFixedAnchor2Raw(self.jid)
    #     cdef np.ndarray[np.float64_t, ndim = 1] np_buff = np.zeros(3)
    #     cdef dReal * res = <dReal *> np_buff.data
    #     res[0] = p[0]
    #     res[1] = p[1]
...
def visDrawWorldYesRender():
    dsDrawWorldinThreadYesRender()


def visDrawWorldWithMuscleWithRef(np.ndarray[np.int32_t, ndim=1] muscle_hold_anchor_num, int anchor_num):
    dsAssignMuscleHoldAnchorNum(<int>muscle_hold_anchor_num.size ,<int>anchor_num, <int*>muscle_hold_anchor_num.data)
    dsDrawWorldinThreadWithMuscleWithRef()


def visDrawWorldOnlyMuscleWithRef(np.ndarray[np.int32_t, ndim=1] muscle_hold_anchor_num, int anchor_num):
    dsAssignMuscleHoldAnchorNum(<int>muscle_hold_anchor_num.size ,<int>anchor_num, <int*>muscle_hold_anchor_num.data)
    dsDrawWorldinThreadOnlyMuscleWithRef()


def visAssignMuscleHoldAnchorNum(np.ndarray[np.int32_t, ndim=1] muscle_hold_anchor_num, int anchor_num):
    dsAssignMuscleHoldAnchorNum(<int>muscle_hold_anchor_num.size ,<int>anchor_num, <int*>muscle_hold_anchor_num.data)


def visAssignMuscleProperty(np.ndarray[np.float32_t, ndim=1] muscle_activation, 
                           np.ndarray[np.float64_t, ndim=1] anchor_pos):
    dsAssignMuscleProperty(<float*>muscle_activation.data, <double*>anchor_pos.data)


def visAssignMusclePropertyWithRef(np.ndarray[np.float32_t, ndim=1] muscle_activation, 
                           np.ndarray[np.float64_t, ndim=1] anchor_pos,
                           np.ndarray[np.float64_t, ndim=1] ref_anchor_pos):
    dsAssignMusclePropertyWithRef(<float*>muscle_activation.data, <double*>anchor_pos.data, <double*> ref_anchor_pos.data)
...
def quat_multiply_backward_fast(np.ndarray q1_, np.ndarray q2_, np.ndarray grad_q_):
    cdef size_t num_quat = q1_.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_q1 = np.zeros((num_quat, 4), dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] grad_q2 = np.zeros((num_quat, 4), dtype=np.float64)
...
def quat_apply_backward_fast(np.ndarray q, np.ndarray v, np.ndarray o_grad):
    cdef size_t num_quat = q.shape[0]
    assert num_quat == v.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] q_grad = np.zeros((num_quat, 4), dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] v_grad = np.zeros((num_quat, 3), dtype=np.float64)
    quat_apply_backward(
        <const double *> q.data,
        <const double *> v.data,
...
def flip_quat_by_w_backward_fast(np.ndarray q, np.ndarray grad_q):
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros_like(grad_q, dtype=np.float64)
...
def quat_to_matrix_backward_fast(np.ndarray q, np.ndarray grad_in):
    cdef size_t num_quat = q.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros_like(q, dtype=np.float64)
...
def vector_to_cross_matrix_backward_fast(np.ndarray vec, np.ndarray grad_in):
    cdef size_t num_vec = vec.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros((num_vec, 3), dtype=np.float64)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quat_to_rotvec_backward_fast(np.ndarray q, np.ndarray angle, np.ndarray grad_in):
    cdef size_t num_quat = q.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros((num_quat, 4), dtype=np.float64)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quat_from_rotvec_backward_fast(np.ndarray x, np.ndarray grad_in):
    cdef size_t num_quat = x.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros((num_quat, 3), dtype=np.float64)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quat_from_matrix_backward_fast(np.ndarray mat, np.ndarray grad_in):
    cdef size_t num_quat = mat.shape[0]
    cdef np.ndarray[np.float64_t, ndim=3] grad_out = np.zeros((num_quat, 3, 3), dtype=np.float64)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def quat_to_hinge_angle_backward_fast(np.ndarray q, np.ndarray axis, np.ndarray grad_in):
    cdef size_t num_quat = q.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] grad_out = np.zeros_like(q, dtype=np.float64)
...
@cython.boundscheck(False)
@cython.wraparound(False)
def parent_child_quat_to_hinge_angle_backward_fast(np.ndarray quat0, np.ndarray quat1,
    np.ndarray init_rel_quat_inv, np.ndarray axis, np.ndarray grad_in):
    cdef size_t num_quat = quat0.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] quat0_grad = np.zeros_like(quat0, dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] quat1_grad = np.zeros_like(quat0, dtype=np.float64)
...
def quat_integrate_backward_fast(np.ndarray q, np.ndarray omega, double dt, np.ndarray grad_in):
    cdef size_t num_quat = q.shape[0]
    cdef np.ndarray[np.float64_t, ndim=2] q_grad = np.zeros((num_quat, 4), dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] omega_grad = np.zeros((num_quat, 3), dtype=np.float64)
...
def vector_normalize_single_backward_fast(np.ndarray x, np.ndarray grad_in):
    cdef size_t ndim = x.shape[0]
    cdef np.ndarray[np.float64_t, ndim=1] grad_out = np.zeros_like(x, dtype=np.float64)
        ...
        def mirror_name_list(name_list: List[str]):
            ...
            for i, n in enumerate(name_list):
                ...
                if n.find('left') == 0:
                    ...
                    idx = index('l' + n[1:])
                #
                elif n.find('Character1_Right') == 0:
                    idx = index('Character1_Left' + n[16:])
                elif n.find('Character1_Left') == 0:
                    idx = index('Character1_Right' + n[15:])
                elif n.find('FootThumbR') == 0:
                    idx = index('FootThumbL')
                elif n.find('FootThumbL') == 0:
                    idx = index('FootThumbR')
                elif n.find('FootPinkyL') == 0:
                    idx = index('FootPinkyR')
                elif n.find('FootPinkyR') == 0:
                    idx = index('FootPinkyL')
...
class pymotionlib:
    ...
    class PyTorchMotionData:
        class PyTorchMotionData:
            """
            Modified from pymotionlib.MotionData.MotionData
            ...
            def to_device(self, device):
                result = pymotionlib.PyTorchMotionData.PyTorchMotionData()
            ...
            def build_from_motion_data(self, motion, device, dtype=torch.float32):
            ...
            def export_to_motion_data(self, is_copy: bool = True):
                result = pymotionlib.MotionData.MotionData()
                result._skeleton_joints = copy.deepcopy(self._skeleton_joints)
                result._skeleton_joint_parents = copy.deepcopy(self._skeleton_joint_parents)
            ...
            def load_rot_trans(self, motion, device, dtype=torch.float32):
            ...
            def compute_joint_global_info(
                ...
                for i, pi in enumerate(self._skeleton_joint_parents):
                    ...
                    joint_position[i] = DiffODE.DiffQuat.quat_apply_imp1(parent_orient, offset_ext) + parent_pos
                    child_orient: torch.Tensor = joint_rotation[:, i, :].contiguous()
                    child_orient_result = DiffODE.DiffQuat.quat_multiply(parent_orient, child_orient)
                    child_orient_result = DiffODE.DiffQuat.quat_normalize(child_orient_result)
                    child_orient_result = DiffODE.DiffQuat.flip_vec_by_dot(child_orient_result)
            ...
            def compute_body_global_info(
                self,
                root_body_translation: torch.Tensor,
                joint_rotation: torch.Tensor,
                joint_tpose: torch.Tensor,
                body_com_tpose: torch.Tensor
            ) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                Modified from pymotionlib.MotionData.MotionData
                compute global information based on given local information
                ...
                root_body_translation: torch.Tensor = root_body_translation.view((-1, 3))
                joint_rotation: torch.Tensor = joint_rotation.view((-1, self._num_joints, 4))


                # print("debug self._num_joints", self._num_joints)

                num_frames, num_bodies = joint_rotation.shape[:2]
                body_position: List[Optional[torch.Tensor]] = [None for _ in range(self._num_joints)]
                body_orientation: List[Optional[torch.Tensor]] = [None for _ in range(self._num_joints)]
                for i, pi in enumerate(self._skeleton_joint_parents):
                    if pi < 0:
                        assert i == 0 and body_position[i] is None and body_orientation[i] is None
                        body_position[i] = root_body_translation
                        body_orientation[i] = joint_rotation[:, i, :].contiguous()
                        continue

                    assert body_position[i] is None and body_orientation[i] is None
                    parent_orient: torch.Tensor = body_orientation[pi]
                    child_orient: torch.Tensor = joint_rotation[:, i, :].contiguous()
                    child_orient_result = DiffODE.DiffQuat.quat_multiply(parent_orient, child_orient)
                    child_orient_result = DiffODE.DiffQuat.quat_normalize(child_orient_result)
                    child_orient_result = DiffODE.DiffQuat.flip_vec_by_dot(child_orient_result)
                    body_orientation[i] = child_orient_result
                    parent_pos: torch.Tensor = body_position[pi]

                    offset0: torch.Tensor = (joint_tpose[i, :] - body_com_tpose[pi, :]).repeat(num_frames, 1)
                    offset1: torch.Tensor = (-joint_tpose[i, :] + body_com_tpose[i, :]).repeat(num_frames, 1)
                    body_position[i] = DiffODE.DiffQuat.quat_apply_imp1(parent_orient, offset0) 
                    body_position[i] += DiffODE.DiffQuat.quat_apply_imp1(child_orient_result, offset1) 
                    body_position[i] += parent_pos

                body_position: torch.Tensor = torch.cat([node[:, None] for node in body_position], dim=1)
                body_orientation: torch.Tensor = torch.cat([node[:, None] for node in body_orientation], dim=1)

                return body_position, body_orientation

            
            def compute_body_global_info_using_joint(
                self,
                root_body_translation: torch.Tensor,
                joint_orientation: torch.Tensor,
                joint_tpose: torch.Tensor,
                body_com_tpose: torch.Tensor
            ) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                ...
                """
                root_body_translation: torch.Tensor = root_body_translation.view((-1, 3))
                joint_orientation: torch.Tensor = joint_orientation.view((-1, self._num_joints, 4))

                num_frames, num_bodies = joint_orientation.shape[:2]
                body_position: List[Optional[torch.Tensor]] = [None for _ in range(self._num_joints)]
                body_orientation: List[Optional[torch.Tensor]] = [None for _ in range(self._num_joints)]
                for i, pi in enumerate(self._skeleton_joint_parents):
                    if pi < 0:
                        assert i == 0 and body_position[i] is None and body_orientation[i] is None
                        body_position[i] = root_body_translation
                        body_orientation[i] = joint_orientation[:, i, :].contiguous()
                        continue

                    assert body_position[i] is None and body_orientation[i] is None
                    parent_orient: torch.Tensor = body_orientation[pi]
                    child_orient: torch.Tensor = joint_orientation[:, i, :].contiguous()
                    body_orientation[i] = child_orient
                    parent_pos: torch.Tensor = body_position[pi]

                    offset0: torch.Tensor = (joint_tpose[i, :] - body_com_tpose[pi, :]).repeat(num_frames, 1)
                    offset1: torch.Tensor = (-joint_tpose[i, :] + body_com_tpose[i, :]).repeat(num_frames, 1)
                    body_position[i] = DiffODE.DiffQuat.quat_apply_imp1(parent_orient, offset0) 
                    body_position[i] += DiffODE.DiffQuat.quat_apply_imp1(child_orient, offset1) 
                    body_position[i] += parent_pos
            ...
            def compute_linear_velocity(self, forward: bool = False) -> torch.Tensor:
                return DiffODE.PyTorchMathHelper.PyTorchMathHelper.vec_diff(self._joint_position, forward, self._fps)
            ...
            def compute_angvel_frag(rotation: torch.Tensor, fps: float) -> torch.Tensor:
                ...
                qw: torch.Tensor = DiffODE.DiffQuat.quat_multiply(qd.view(-1, 4), q_conj)
            ...
            def compute_angvel_base(rotation: torch.Tensor, fps: float, forward: bool = False) -> torch.Tensor:
                frag: torch.Tensor = pymotionlib.PyTorchMotionData.PyTorchMotionData.compute_angvel_frag(rotation, fps)
                if forward:
...
class DiffODE:
    class PyTorchMathHelper:
        class PyTorchMathHelper:
            """
            Math Helper use pytorch
            """
            def __init__(self):
                pass

            @staticmethod
            def vec_axis_to_zero(v: torch.Tensor, axis: Union[int, List[int], torch.Tensor]) -> torch.Tensor:
                with torch.no_grad():
                    mask = torch.ones_like(v)
                    mask[..., axis] = 0
                result: torch.Tensor = mask * v
                return result

            @staticmethod
            def clone_func(x: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
                return x.detach().clone() if x is not None else None

            @staticmethod
            def detach_func(x: Optional[torch.Tensor]):
                if x is not None:
                    x.detach_()

            @staticmethod
            def unit_vector(axis: int) -> torch.Tensor:
                return torch.as_tensor(Common.MathHelper.MathHelper.unit_vector(axis))

            @staticmethod
            def unit_quat(dtype=torch.float64) -> torch.Tensor:
                """
                unit quaternion == (0, 0, 0, 1)
                """
                return torch.as_tensor([0, 0, 0, 1], dtype=dtype)

            @staticmethod
            def unit_quat_arr(shape: Union[int, Iterable, Tuple[int]]) -> torch.Tensor:
                """
                unit quaternion in (x, y, z, w)
                """
                return torch.from_numpy(Common.MathHelper.MathHelper.unit_quat_arr(shape))
            ...
            @staticmethod   # Test OK
            def clip(x: torch.Tensor, min_val: Union[float, torch.Tensor], max_val: Union[float, torch.Tensor]) -> torch.Tensor:
                if type(min_val) == float:
                    min_val: torch.Tensor = torch.full(x.shape, min_val, dtype=x.dtype)
                if type(max_val) == float:
                    max_val: torch.Tensor = torch.full(x.shape, max_val, dtype=x.dtype)

                min_val: torch.Tensor = min_val.view(x.shape)
                max_val: torch.Tensor = max_val.view(x.shape)

                res = torch.minimum(torch.maximum(x, min_val), max_val)
                return res
                # document at https://pytorch.org/docs/master/generated/torch.clamp.html#torch.clamp
                # min(max(xi, min_value_i), max_value_i)

            @staticmethod
            def insert_at(x: torch.Tensor, ins: Optional[torch.Tensor], idx: int) -> torch.Tensor:
                """
                insert ins into x at idx
                """
                if ins is None:
                    ins = torch.zeros((1,) + x.shape[1:], dtype=x.dtype)

                assert x.shape[1:] == ins.shape[1:]
                if idx == 0:
                    res = torch.cat([ins, x], dim=0)
                elif idx == x.shape[0]:
                    res = torch.cat([x, ins], dim=0)
                else:
                    res = torch.cat([x[:idx], ins, x[idx:]], dim=0)

                return res

            @staticmethod
            def delete_at(x: torch.Tensor, idx: int) -> torch.Tensor:
                """
                delete at position idx in dim 0.
                return torch.Tensor in shape (x.shape[0]-1, ...)
                """
                if idx == 0:
                    return x[1:]
                    ...
                    res = torch.cat([x[0:idx], x[idx+1:]], dim=0)
                    return res

            @staticmethod
            def decompose_rotation(q: torch.Tensor, vb: torch.Tensor) -> torch.Tensor:
                """
                Same as implementation in Common.MathHelper
                """
                vb: torch.Tensor = vb.view(-1, 3)
                if vb.shape[:-1] < q.shape[:-1]:  # broadcast manually
                    vb = vb.repeat((q.shape[0] // vb.shape[0], 1))
                va: torch.Tensor = DiffODE.DiffQuatquat_apply_imp1(q, vb)
                va = DiffODE.DiffQuat.vec_normalize(va)

                rot_axis: torch.Tensor = torch.cross(va, vb, dim = 1)
                rot_axis = DiffODE.DiffQuat.vec_normalize(rot_axis)

                # TODO: minus or plus..?
                rot_angle: torch.Tensor = -torch.arccos(torch.clip(torch.sum(va * vb, dim=-1), -1+1e-7, 1-1e-7))
                if len(rot_angle.shape) > 0:
                    rot_angle = rot_angle[:, None]

                tmp: torch.Tensor = DiffODE.DiffQuat.quat_from_rotvec(rot_angle * (-rot_axis))
                res: torch.Tensor = DiffODE.DiffQuat.quat_multiply(tmp, q)
                res = DiffODE.DiffQuat.quat_normalize(res)
                return res

            @staticmethod
            def xyz_decompose(q: torch.Tensor, axis: int):
                return DiffODE.PyTorchMathHelper.PyTorchMathHelper.axis_decompose(q, torch.as_tensor(Common.MathHelper.MathHelper.unit_vector(axis), dtype = q.dtype, device = q.device))

            @staticmethod
            def axis_decompose(q: torch.Tensor, axis: torch.Tensor):
                """
                return:
                res: rotation along axis
                r_other:
                """
                assert axis.shape == (3,)
                res: torch.Tensor = DiffODE.PyTorchMathHelper.PyTorchMathHelper.decompose_rotation(q, axis)  # res: rotation at given axis is 0..
                r_other = DiffODE.DiffQuat.flip_quat_by_w(DiffODE.DiffQuat.quat_multiply(DiffODE.DiffQuat.quat_inv(q), res))
                res = DiffODE.DiffQuat.quat_normalize(res)
                r_other = DiffODE.DiffQuat.quat_normalize(r_other)

                return res, r_other

            @staticmethod
            def x_decompose(q: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                decompose quaternion into 2 parts:
                1. quaternion with only x part
                2. quaternion with only y, z part
                """
                return DiffODE.PyTorchMathHelper.PyTorchMathHelper.xyz_decompose(q, 0)

            @staticmethod
            def y_decompose(q: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                decompose quaternion into 2 parts:
                1. quaternion with only y part
                2. quaternion with only x, z part
                """
                return DiffODE.PyTorchMathHelper.PyTorchMathHelper.xyz_decompose(q, 1)

            @staticmethod
            def z_decompose(q: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                decompose quaternion into 2 parts
                1. quaternion with only z part
                2. quaternion with only x, y part
                """
                return DiffODE.PyTorchMathHelper.PyTorchMathHelper.xyz_decompose(q, 2)

            @staticmethod
            def quat_equal(qa: torch.Tensor, qb: torch.Tensor):
                return Common.MathHelper.MathHelper.quat_equal(qa.detach().numpy(), qb.detach().numpy())

            @staticmethod
            def quat_between(a: torch.Tensor, b: torch.Tensor):  # Test OK
                """
                Rotation from vector a to vector b
                :param a: (n, 3) vector
                :param b: (n, 3) vector
                :return: (n, 4) quaternion
                """
                cross_res: torch.Tensor = torch.cross(a, b, dim = 1)
                w_: torch.Tensor = torch.sqrt((a ** 2).sum(dim=-1) * (b ** 2).sum(dim=-1)) + (a * b).sum(dim=-1)
                res_: torch.Tensor = torch.cat([cross_res, w_[..., None]], dim=-1)
                res: torch.Tensor = res_ / torch.linalg.norm(res_, dim=-1, keepdim=True)
                return res

            @staticmethod
            def vec_diff(v_in: torch.Tensor, forward: bool, fps: float):  # Test OK
                """

                """
                frag: torch.Tensor = torch.diff(v_in, dim=0) * fps
                if forward:
                    res = torch.cat([frag, frag[None, -1]], dim=0)
                else:
                    res = torch.cat([frag[None, 0], frag], dim=0)
                return res

            @staticmethod
            def xz_vector_to_xyz(xz: torch.Tensor) -> torch.Tensor:
                x: torch.Tensor = xz[..., 0, None]
                z: torch.Tensor = xz[..., 1, None]
                return torch.cat([x, torch.zeros_like(x), z], dim=-1)

            @staticmethod
            def flip_quat_pair_by_dot(q0s: torch.Tensor, q1s: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
                """
                q0 will not be changed.
                q1 will be flipped to the same semi sphere as q0
                TODO
                ...
                dot_value: torch.Tensor = torch.sum(q0s * q1s, dim=-1, keepdim=True) < 0
                dot_res: torch.Tensor = torch.cat([dot_value] * 4, dim=-1)
                # flip q1s by mask..
                q1: np.ndarray = q1s.copy()
                q1[dot_res] = -q1[dot_res]
                return q0s, q1
    ...
    class ODEUtils:
        ...
        def d_plane_space_ge(n0: torch.Tensor, n1: torch.Tensor, n2: torch.Tensor, idx_ge: torch.Tensor, use_all = False):
            """
            Case:
            """
            # print("ge")
            if not use_all:
                n0_ge: torch.Tensor = n0[idx_ge, None]
                n1_ge: torch.Tensor = n1[idx_ge, None]
                n2_ge: torch.Tensor = n2[idx_ge, None]
            else:
                n0_ge,n1_ge,n2_ge = n0.view(-1,1),n1.view(-1,1),n2.view(-1,1)
            a_ge: torch.Tensor = n1_ge ** 2 + n2_ge ** 2
            # assert torch.sum(torch.isnan(a_ge)) == 0 and torch.all(a_ge > 0)
            k_ge: torch.Tensor = torch.pow(a_ge,-0.5)#1.0 / torch.sqrt(a_ge)
            p_ge: torch.Tensor = torch.cat([torch.zeros_like(n1_ge, requires_grad=True), -n2_ge * k_ge, n1_ge * k_ge], dim=-1)
            q_ge: torch.Tensor = torch.cat([a_ge * k_ge, -n0_ge * p_ge[..., 2:3], n0_ge * p_ge[..., 1:2]], dim=-1)

            return p_ge, q_ge


        def d_plane_space_leq(n0: torch.Tensor, n1: torch.Tensor, n2: torch.Tensor, idx_leq: torch.Tensor, use_all = False):
            """
            Case:
            """
            # print("leq")
            if not use_all:
                n0_leq: torch.Tensor = n0[idx_leq, None]
                n1_leq: torch.Tensor = n1[idx_leq, None]
                n2_leq: torch.Tensor = n2[idx_leq, None]
            else:
                n0_leq, n1_leq, n2_leq = n0.view(-1, 1), n1.view(-1, 1), n2.view(-1, 1)

            a_leq: torch.Tensor = n0_leq ** 2 + n1_leq ** 2
            # assert torch.sum(torch.isnan(a_leq)) == 0 and torch.all(a_leq > 0)
            k_leq: torch.Tensor = torch.pow(a_leq, -0.5)  # 1.0 / torch.sqrt(a_leq)

            zero_leq: torch.Tensor = torch.zeros_like(n0_leq)
            p_leq: torch.Tensor = torch.cat([-n1_leq * k_leq, n0_leq * k_leq, zero_leq], dim=-1)
            q_leq: torch.Tensor = torch.cat([-n2_leq * p_leq[..., 1:2], n2_leq * p_leq[..., 0:1], a_leq*k_leq], dim=-1)

            return p_leq, q_leq


        def _d_plane_space_multi_pq(n: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            """
            input: n: normal vector in shape (*, 3)
            output: 2 orthogonal vectors of n in shape (*, 3), (*, 3)
            same as C++ open dynamics engine code..
            """
            assert n.shape[-1] == 3
            ...
            n1: torch.Tensor = n[..., 1]
            n2: torch.Tensor = n[..., 2]
            # n0,n1,n2 = torch.tensor_split(n,3,dim=-1)s

            # warning cannot ignored...
            idx_ge: torch.BoolTensor = torch.abs(n2) > math.sqrt(0.5)  # idx_ge.requires_grad==False, it's OK
            if torch.all(idx_ge):
                return DiffODE.ODEUtils.d_plane_space_ge(n0, n1, n2, idx_ge, use_all= True)

            idx_leq: torch.Tensor = ~idx_ge  # idx_leq.requires_grad==False, it's OK
            if torch.all(idx_leq):
                return DiffODE.ODEUtils.d_plane_space_leq(n0, n1, n2, idx_leq, use_all=True)

            ge_p, ge_q = DiffODE.ODEUtils.d_plane_space_ge(n0, n1, n2, idx_ge)
            leq_p, leq_q = DiffODE.ODEUtils.d_plane_space_leq(n0, n1, n2, idx_leq)

            res_p: torch.Tensor = DiffODE.ODEUtils.CatWithMask.apply(ge_p, leq_p, idx_ge)
            res_q: torch.Tensor = DiffODE.ODEUtils.CatWithMask.apply(ge_q, leq_q, idx_ge)
            # res_p1 = torch.scatter(torch.zeros())
            return res_p, res_q


        def dplane_space_multi_pq(n: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            """
            input: n: normal vector
            same as C++ open dynamics engine code..
            """
            n_flat = n.view(n.shape[:-1]) if n.shape[-1] == 1 else n
            assert n_flat.shape[-1] == 3

            p, q = DiffODE.ODEUtils._d_plane_space_multi_pq(n_flat)
            return p.view(n.shape), q.view(n.shape)


        def test_d_plane_multi():
            x = torch.rand((40, 3), requires_grad=True)
            x = Variable((x / torch.linalg.norm(x, dim=-1, keepdims=True)).detach(), requires_grad=True)
            p, q = DiffODE.ODEUtils.dplane_space_multi_pq(x)
            tmp = torch.sum(q)
            tmp.backward()
            print(x.grad)

    class DiffQuat:
        class QuatMultiplyFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any):
                q1: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                q2: np.ndarray = np.ascontiguousarray(args[1].detach().numpy(), dtype=np.float64)
                ctx.q1 = q1
                ctx.q2 = q2
                result: np.ndarray = quat_multiply_forward_fast(q1, q2)
                result: torch.Tensor = torch.as_tensor(result, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad or args[1].requires_grad
                return result

            @staticmethod
            def backward(ctx: Any, *grad_outputs: Any) -> Tuple[torch.Tensor, torch.Tensor]:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q1, grad_q2 = quat_multiply_backward_fast(ctx.q1, ctx.q2, out_grad)
                ctx.q1 = None
                ctx.q2 = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_q1, dtype=dtype), torch.as_tensor(grad_q2, dtype=dtype)


        class QuatApplyFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                q: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                v: np.ndarray = np.ascontiguousarray(args[1].detach().numpy(), dtype=np.float64)
                ctx.in_q = q
                ctx.in_v = v
                result: np.ndarray = quat_apply_forward_fast(q, v)
                result: torch.Tensor = torch.as_tensor(result, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad or args[1].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> Tuple[torch.Tensor, torch.Tensor]:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q, grad_v = quat_apply_backward_fast(ctx.in_q, ctx.in_v, out_grad)
                ctx.in_q = None
                ctx.in_v = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_q, dtype=dtype), torch.as_tensor(grad_v, dtype=dtype)


        class QuatFlipByWFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                q: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                ctx.in_q = q
                result: np.ndarray = flip_quat_by_w_fast(q)
                result: torch.Tensor = torch.from_numpy(result).type(args[0].dtype)
                result.requires_grad = args[0].requires_grad
                return result

            @staticmethod
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q = flip_quat_by_w_backward_fast(ctx.in_q, out_grad)
                ctx.in_q = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_q, dtype=dtype)


        class QuatToMatrixFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                q: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                ctx.in_q = q
                result: np.ndarray = quat_to_matrix_fast(q)
                result: torch.Tensor = torch.as_tensor(result, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q = quat_to_matrix_backward_fast(ctx.in_q, out_grad)
                ctx.in_q = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_q, dtype=dtype)


        class VecToCrossMatrix(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                v: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                ctx.in_v = v
                result: np.ndarray = vector_to_cross_matrix_fast(v)
                result: torch.Tensor = torch.as_tensor(result, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_v = vector_to_cross_matrix_backward_fast(ctx.in_v, out_grad)
                ctx.in_v = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_v, dtype=dtype)


        class QuatFromRotVecFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                x: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                ctx.in_x = x
                quat: np.ndarray = quat_from_rotvec_fast(x)
                result = torch.as_tensor(quat, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad = quat_from_rotvec_backward_fast(ctx.in_x, out_grad)
                ctx.in_x = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad, dtype=dtype)


        class QuatToRotvecFast(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                q: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                ctx.in_q = q
                angle, rotvec = quat_to_rotvec_fast(q)
                ctx.in_angle = angle
                result: torch.Tensor = torch.as_tensor(rotvec, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q = quat_to_rotvec_backward_fast(ctx.in_q, ctx.in_angle, out_grad)
                ctx.in_q = None
                ctx.in_angle = None
                dtype = grad_outputs[0].dtype
                return torch.as_tensor(grad_q, dtype=dtype)
        ...
        class QuatToHingeAngle(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                q = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                axis = np.ascontiguousarray(args[1].detach().numpy(), dtype=np.float64)
                ctx.in_q = q
                ctx.in_axis = axis
                angle: np.ndarray = quat_to_hinge_angle_fast(q, axis)
                result: torch.Tensor = torch.from_numpy(angle)
                result.requires_grad = args[0].requires_grad or args[1].requires_grad
                return result

            @staticmethod
            def backward(ctx: Any, *grad_outputs: Any) -> Tuple[torch.Tensor, None]:
                # we don't need gradient for axis in compute hinge angle in DiffODE
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q = quat_to_hinge_angle_backward_fast(ctx.in_q, ctx.in_axis, out_grad)
                ctx.in_q = None
                ctx.in_axis = None
                return torch.from_numpy(grad_q), None


        class QuatToHingeAngleParentChild(Function):
            """
            quat0, quat1, init_rel_quat_inv, axis
            """
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                quat0 = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                quat1 = np.ascontiguousarray(args[1].detach().numpy(), dtype=np.float64)
                init_rel_quat_inv = np.ascontiguousarray(args[2].detach().numpy(), dtype=np.float64)
                axis = np.ascontiguousarray(args[3].detach().numpy(), dtype=np.float64)
                angle: np.ndarray = parent_child_quat_to_hinge_angle_fast(quat0, quat1, init_rel_quat_inv, axis)
                ctx.in_quat0 = quat0
                ctx.in_quat1 = quat1
                ctx.in_init_rel_quat_inv = init_rel_quat_inv
                ctx.in_axis = axis
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad0, grad1 = parent_child_quat_to_hinge_angle_backward_fast(ctx.in_quat0, ctx.in_quat1, ctx.in_init_rel_quat_inv, ctx.in_axis, out_grad)
                ctx.in_quat0 = None
                ctx.in_quat1 = None
                ctx.in_init_rel_quat_inv = None
                ctx.in_axis = None
                return torch.from_numpy(grad0), torch.from_numpy(grad1), None, None


        class QuatIntegrate(Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                quat: np.ndarray = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                omega: np.ndarray = np.ascontiguousarray(args[1].detach().numpy(), dtype=np.float64)
                t = args[2].item()
                ctx.in_quat = quat
                ctx.in_omega = omega
                ctx.in_t = t
                result = quat_integrate_fast(quat, omega, t)
                result: torch.Tensor = torch.as_tensor(result, dtype=args[0].dtype)
                result.requires_grad = args[0].requires_grad or args[1].requires_grad
                return result
            ...
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                out_grad: np.ndarray = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                grad_q, grad_omega = quat_integrate_backward_fast(ctx.in_quat, ctx.in_omega, ctx.in_t, out_grad)
                ctx.in_quat = None
                ctx.in_omega = None
                ctx.in_t = None
                return torch.as_tensor(grad_q), torch.as_tensor(grad_omega), None


        class TestNormalize(torch.autograd.Function):
            @staticmethod
            def forward(ctx: Any, *args: Any) -> torch.Tensor:
                x = np.ascontiguousarray(args[0].detach().numpy(), dtype=np.float64)
                res = vector_normalize_single_fast(x)
                ctx.in_x = x
            ...
            @staticmethod
            def backward(ctx: Any, *grad_outputs: Any) -> torch.Tensor:
                grad_in = np.ascontiguousarray(grad_outputs[0].detach().numpy(), dtype=np.float64)
                res = vector_normalize_single_backward_fast(ctx.in_x, grad_in)
                ctx.in_x = None
        ...
        def quat_multiply(p: torch.Tensor, q: torch.Tensor) -> torch.Tensor:
            ...
            if p.device != torch.device("cpu"):
                ...
                return DiffODE.DiffQuat.QuatMultiplyFast.apply(p, q)
        ...
        def quat_apply_ori(q: torch.Tensor, vec3: torch.Tensor) -> torch.Tensor:
            ...
            tmp: torch.Tensor = DiffODE.DiffQuat.quat_multiply(q, ext_vec)  # shape = (num quat, 4)
            tmp2: torch.Tensor = DiffODE.DiffQuat.quat_multiply(tmp, DiffODE.DiffQuat.quat_inv(q))  # shape == (num quat, 4)
        ...
        def quat_apply(q: torch.Tensor, vec3: torch.Tensor) -> torch.Tensor:
            ...
            if q.device != torch.device("cpu"):
                ...
                return DiffODE.DiffQuat.QuatApplyFast.apply(q, vec3)
        ...
        def vec_normalize(x: torch.Tensor) -> torch.Tensor:
            ...
            """
            result = x / torch.linalg.norm(x, 2, -1, keepdim=True).clamp(min=1e-9)
            return result
        ...
        def quat_normalize(q: torch.Tensor) -> torch.Tensor:
            ...
            """
            return DiffODE.DiffQuat.vec_normalize(q)
        ...
        def quat_integrate(q: torch.Tensor, omega: torch.Tensor, dt: float) -> torch.Tensor:
            ...
            if q.device != torch.device("cpu"):
                ...
                delta_q = 0.5 * dt * DiffODE.DiffQuat.quat_multiply(omega, q)
                result = q + delta_q
                result = DiffODE.DiffQuat.quat_normalize(result)
            else:
                result = DiffODE.DiffQuat.QuatIntegrate.apply(q, omega, torch.as_tensor(dt, dtype=torch.float64))
        ...
        def quat_diff(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
            ...
            a_inv = DiffODE.DiffQuat.quat_inv(a)
            result = DiffODE.DiffQuat.quat_multiply(a_inv, b)
            return result


        def log_quat_diff_sqr(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:  # TODO: test
            # log(a^{-1} * b)
            a_inv: torch.Tensor = DiffODE.DiffQuat.quat_inv(a)
            a_inv_b: torch.Tensor = DiffODE.DiffQuat.quat_multiply(a_inv, b)
            rotvec: torch.Tensor = DiffODE.DiffQuat.quat_to_rotvec(a_inv_b)
            sqr_theta: torch.Tensor = torch.sum(rotvec ** 2, dim=-1)
            return sqr_theta


        def log_quat_diff(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
            sqr_theta: torch.Tensor = DiffODE.DiffQuat.log_quat_diff_sqr(a, b)
        ...
        def quat_to_matrix(q: torch.Tensor, do_normalize: bool = False) -> torch.Tensor:
            ...
            if do_normalize:
                q = DiffODE.DiffQuat.quat_normalize(q)
            ...
            if q.device != torch.device("cpu"):
                ...
                res: torch.Tensor = DiffODE.DiffQuat.QuatToMatrixFast.apply(q).view(origin_shape[:-1] + (3, 3))
        ...
        def quat_from_matrix(matrix: torch.Tensor) -> torch.Tensor:
            ...
            if len(ind > 0):
                ...
                quat_neq3 = DiffODE.DiffQuat._QuatCatFunc.apply(quat_neq3_i, quat_neq3_j, quat_neq3_k, quat_neq3_3, i, j, k)
            ...
            if quat_neq3 is None:
                ...
                quat: torch.Tensor = DiffODE.ODEUtils.CatWithMask.apply(quat_neq3, quat_eq3, flg)

            quat: torch.Tensor = DiffODE.DiffQuat.quat_normalize(quat)
        ...
        def vector_to_cross_matrix(x: torch.Tensor) -> torch.Tensor:
            ...
            if x.device != torch.device("cpu"):
                ...
                mat: torch.Tensor = torch.stack((
                    zero00, -x2, x1,
                    x2, zero11, -x0,
                    -x1, x0, zero22
                ), dim=-1).view(*x.shape[:-2], 3, 3)
            else:
                mat: torch.Tensor = DiffODE.DiffQuat.VecToCrossMatrix.apply(x.reshape(-1, 3)).view(x.shape[:-2] + (3, 3))

            return mat


        def quat_from_rotvec_slow(rotvec: torch.Tensor) -> torch.Tensor:
            ...
            if torch.any(small_angle):
                scale_small = (0.5 - norms[small_angle] ** 2 / 48 + norms[small_angle] ** 4 / 3840)
            if torch.any(large_angle):
                scale_large = (torch.sin(norms[large_angle] / 2) / norms[large_angle])
            ...
            if scale_small is None:
                ...
                scale = DiffODE.DiffQuat.CatWithMask.apply(scale_small, scale_large, small_angle)
        ...
        def quat_from_rotvec(rotvec: torch.Tensor) -> torch.Tensor:
            ...
            """
            if rotvec.shape == (3,):
                rotvec: torch.Tensor = rotvec[None, :]

            if rotvec.device != torch.device("cpu"):
                quat = DiffODE.DiffQuat.quat_from_rotvec_slow(rotvec)
            else:
                quat: torch.Tensor = DiffODE.DiffQuat.QuatFromRotVecFast.apply(rotvec)
        ...
        def quat_from_vec_and_angle(axis: torch.Tensor, angle: torch.Tensor) -> torch.Tensor:
            ...
            axis: torch.Tensor = DiffODE.DiffQuat.vec_normalize(axis)
        ...
        def flip_quat_by_w(q: torch.Tensor) -> torch.Tensor:
            ...
            if q.device !=  torch.device("cpu"):
                ...
                res = DiffODE.DiffQuat.QuatFlipByWFast.apply(q.reshape(-1, 4)).view(q.shape)
            return res
        ...
        def flip_quat_by_dot(q: torch.Tensor):
            ...
            """
            return DiffODE.DiffQuat.flip_vec_by_dot(q)


        def quat_to_rotvec_slow(q: torch.Tensor) -> torch.Tensor:
            quat: torch.Tensor = DiffODE.DiffQuat.flip_quat_by_w(q)
            angle: torch.Tensor = torch.as_tensor(2.0, dtype=q.dtype) * torch.atan2(torch.linalg.norm(quat[:, :3], dim=1), quat[:, 3])
            ...
            scale_small = scale_large = None
            if torch.any(small_angle):
                scale_small = (2 + angle[small_angle] ** 2 / 12 + 7 * angle[small_angle] ** 4 / 2880)
            if torch.any(large_angle):
                scale_large = angle[large_angle] / torch.sin(angle[large_angle] / 2)
            if scale_small is None:
                scale = scale_large
            elif scale_large is None:
                scale = scale_small
            else:
                scale = DiffODE.ODEUtils.CatWithMask.apply(scale_small, scale_large, small_angle)
        ...
        def quat_to_rotvec(q: torch.Tensor, do_normalize: bool = False):  # Test OK
            ...
            if q.device !=  torch.device("cpu"):
                rotvec = DiffODE.DiffQuat.quat_to_rotvec_slow(q)
            else:
                rotvec: torch.Tensor = DiffODE.DiffQuat.QuatToRotvecFast.apply(q.view(-1, 4))
        ...
        def quat_to_vec6d(q: torch.Tensor, do_normalize: bool = False) -> torch.Tensor:
            assert q.shape[-1] == 4
            mat: torch.Tensor = DiffODE.DiffQuat.quat_to_matrix(q, do_normalize)
        ...
        def normalize_vec6d_cat(x: torch.Tensor) -> torch.Tensor:
            first_col, second_col, last_col = DiffODE.DiffQuat.normalize_vec6d(x)
            result: torch.Tensor = torch.cat([first_col[..., None], second_col[..., None]], dim=-1)
            return result


        class NormalizeVec6dLayer(nn.Module):
            def __init__(self) -> None:
                super().__init__()

            def forward(self, x: torch.Tensor) -> torch.Tensor:
                raw_shape = x.shape
                first_col, second_col, last_col = DiffODE.DiffQuat.normalize_vec6d(x.view(-1, 3, 2))
                x: torch.Tensor = torch.cat([first_col[..., None], second_col[..., None]], dim=-1)
        ...
        def vec6d_to_quat(x: torch.Tensor) -> torch.Tensor:
            first_col, second_col, last_col = DiffODE.DiffQuat.normalize_vec6d(x)
            mat: torch.Tensor = torch.cat([first_col[..., None], second_col[..., None], last_col[..., None]], dim=-1)
            quat: torch.Tensor = DiffODE.DiffQuat.quat_from_matrix(mat.view(-1, 3, 3)).view(x.shape[:-2] + (4,))
            return quat
        ...
        def diff_angle_between_vec6d(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
            ...
            x_mat = torch.cat([node[..., None] for node in DiffODE.DiffQuat.normalize_vec6d(x)], dim=-1)
            y_mat = torch.cat([node[..., None] for node in DiffODE.DiffQuat.normalize_vec6d(y)], dim=-1)
            return DiffODE.DiffQuat.diff_angle_bewteen_matrix(x_mat, y_mat)


        def diff_angle_bewteen_matrix(x_mat: torch.Tensor, y_mat: torch.Tensor) -> torch.Tensor:
            x_mat_inv = DiffODE.DiffQuat.matrix_inv(x_mat)
            mat_dup = x_mat_inv @ y_mat
            mat_angle = DiffODE.DiffQuat.matrix_to_angle(mat_dup)
            return mat_angle
        ...
        def normalize_pred_rot(y_pred: torch.Tensor, rotate_type: Common.RotateType) -> torch.Tensor:
            """
            the output of rotation should be normalized.
            """
            if rotate_type == Common.RotateType.Vec6d:
                y_pred = DiffODE.DiffQuat.normalize_vec6d_cat(y_pred)
            elif rotate_type == Common.RotateType.SVD9d:
                y_pred = DiffODE.DiffQuat.symmetric_orthogonalization(y_pred)
            elif rotate_type == Common.RotateType.AxisAngle:
                y_pred = y_pred
            elif rotate_type == Common.RotateType.Quaternion:
                y_pred = DiffODE.DiffQuat.quat_normalize(y_pred)
            else:
                raise NotImplementedError
        ...
        def compute_delta_angle(y_pred: torch.Tensor, y_gt: torch.Tensor, rotate_type: Common.RotateType):
            if rotate_type == Common.RotateType.Vec6d:
                delta_angle = torch.abs(DiffODE.DiffQuat.diff_angle_between_vec6d(y_pred, y_gt))
            elif rotate_type == Common.RotateType.SVD9d or rotate_type == Common.RotateType.Matrix:
                delta_angle = torch.abs(DiffODE.DiffQuat.diff_angle_bewteen_matrix(y_pred, y_gt))
            elif rotate_type == Common.RotateType.AxisAngle:
        ...
        def convert_to_quat(x: torch.Tensor, rotate_type: Common.RotateType) -> torch.Tensor:
            if rotate_type == Common.RotateType.SVD9d:
                quat = DiffODE.DiffQuat.quat_from_matrix(x.view(-1, 3, 3)).view(x.shape[:-2] + (4,))
            elif rotate_type == Common.RotateType.Matrix:
                quat = DiffODE.DiffQuat.quat_from_matrix(x.view(-1, 3, 3)).view(x.shape[:-2] + (4,))
            elif rotate_type == Common.RotateType.Vec6d:
                quat = DiffODE.DiffQuat.vec6d_to_quat(x)
            elif rotate_type == Common.RotateType.Quaternion:
                quat = x
            else:
                raise NotImplementedError
        ...
        def quat_to_other_rotate(q: torch.Tensor, rotate_type: Common.RotateType) -> torch.Tensor:
            origin_shape = q.shape
            q: torch.Tensor = q.view(-1, 4)
            if rotate_type == Common.RotateType.SVD9d or rotate_type == Common.RotateType.Matrix:
                ret: torch.Tensor = DiffODE.DiffQuat.quat_to_matrix(q)
            elif rotate_type == Common.RotateType.AxisAngle:
                ret: torch.Tensor = DiffODE.DiffQuat.quat_to_rotvec(q)
            elif rotate_type == Common.RotateType.Vec6d:
                ret: torch.Tensor = DiffODE.DiffQuat.quat_to_vec6d(q)
            elif rotate_type == Common.RotateType.Quaternion:
                ret: torch.Tensor = q
            else:
                raise NotImplementedError
...
class ODESim:
    ...
    class BodyInfo:
        ...
        class BodyInfo:
            ...
            def get_geoms0_gids(self):
                return np.array([body.geoms[0].get_gid() for body in self.bodies],dtype=np.uint64)
            ...
            # Slow version
            def get_global_euler_XYZ_axis(self, joint_c_id) -> np.ndarray:
                """
                joint' euler axis in world coordinate
                In T pose, the Body's quat and Joint's quat are set to the same (0, 0, 0, 1)
                shape: (3, n_q)
                """
                parent_qs, _, c_local_qs, _ = self.world.get_all_joint_local_angle(joint_c_id)
                child_qs = np.concatenate((self.bodies[0].getQuaternionScipy().reshape(1, 4), c_local_qs), axis=0)
                child_euler_XYZ = Rotation.from_quat(child_qs).as_euler('XYZ')
                child_euler_x = np.zeros([child_qs.shape[0], 3])
                child_euler_y = np.zeros([child_qs.shape[0], 3])
                child_euler_z = np.zeros([child_qs.shape[0], 3])
                res = np.zeros([3, 3 * child_qs.shape[0]])
                res[:, 0] = np.array([1., 0., 0.])
                res[:, 1] = Rotation.from_euler('XYZ', [child_euler_XYZ[0, 0], 0., 0.]).as_matrix() @ np.array([0., 1., 0.])
                res[:, 2] = Rotation.from_euler('XYZ', [child_euler_XYZ[0, 0], 0., 0.]).as_matrix()@ Rotation.from_euler('XYZ', [0., child_euler_XYZ[0, 1], 0.]).as_matrix() @ np.array([0., 0., 1.])
                for i in range(1, child_qs.shape[0]):
                    child_euler_x[i] = Rotation.from_quat(parent_qs[i-1]).as_matrix() @ np.array([1., 0., 0.])
                    child_euler_y[i] = Rotation.from_quat(parent_qs[i-1]).as_matrix() @ Rotation.from_euler('XYZ', [child_euler_XYZ[i, 0], 0., 0.]).as_matrix() @ np.array([0., 1., 0.])
                    child_euler_z[i] = Rotation.from_quat(parent_qs[i-1]).as_matrix() @ Rotation.from_euler('XYZ', [child_euler_XYZ[i, 0], 0., 0.]).as_matrix()@ Rotation.from_euler('XYZ', [0., child_euler_XYZ[i, 1], 0.]).as_matrix() @ np.array([0., 0., 1.])
                    res[:, 3 * i + 0] = child_euler_x[i]
                    res[:, 3 * i + 1] = child_euler_y[i]
                    res[:, 3 * i + 2] = child_euler_z[i]
                return res
    ...
    class ODECharacter:
        ...
        class ODECharacter:
            def __init__(self, world: World, space: SpaceBase):
                ...
                self._is_enable_just_right_arm = False
            ...
            @property
            def is_enable_just_right_arm(self) -> bool:
                return self._is_enable_just_right_arm

            @is_enable.setter
            def is_enable_just_right_arm(self, value: bool):
                if self._is_enable_just_right_arm == value:
                    return
                ## TODO : change the body and joint names using API
                body_right_arm_names = [ 'rClavicle', 'rUpperArm', 'rLowerArm', 'rHand']
                joint_right_arm_names = [ 'Character1_RightShoulder', 'Character1_RightArm', 'Character1_RightForeArm', 'Character1_RightHand']
                if value:
                    for body in self.bodies:
                        body.disable()
                        for geom in body.geom_iter():
                    ...
                    for joint in self.joints:
                        joint.disable()
                    for body in self.bodies:
                        if body.name in body_right_arm_names:
                            body.enable()
                            for geom in body.geom_iter():
                                geom.enable()
                    for joint in self.joints:
                        if joint.name in joint_right_arm_names:    
                            joint.enable()
                else:
                    for body in self.bodies:
                        body.enable()
    ...
    class PDController:
        ...
        class DampedPDControler:
            ...
            def get_torques_by_quat(self, tar_local_qs: np.ndarray) -> np.ndarray:
                c_local_torque, c_global_torque, tot_power = self.world.get_pd_control_torque(self.joint_c_id, tar_local_qs, self.kps, self.tor_lim)
                self.cache_global_torque = c_global_torque
                self.cache_local_torque = c_local_torque
                self.character.accum_energy += tot_power
            ...
            def add_torques_by_quat_with_fatigue(self, tar_local_qs: np.ndarray, joint_capacity) -> np.ndarray:
                c_local_torque, c_global_torque, tot_power = self.world.get_pd_control_torque_with_fatigue(self.joint_c_id, tar_local_qs, self.kps, self.tor_lim, joint_capacity)
                self.cache_global_torque = c_global_torque
                self.cache_local_torque = c_local_torque
                self.world.add_global_torque(c_global_torque, self.joint_info.parent_body_c_id, self.joint_info.child_body_c_id)
                self.character.accum_energy += tot_power
            ...
            def get_torques_by_quat_with_fatigue(self, tar_local_qs: np.ndarray, joint_capacity) -> np.ndarray:
                c_local_torque, c_global_torque, tot_power = self.world.get_pd_control_torque_with_fatigue(self.joint_c_id, tar_local_qs, self.kps, self.tor_lim, joint_capacity)
                self.cache_global_torque = c_global_torque
                self.cache_local_torque = c_local_torque
                self.character.accum_energy += tot_power
    ...
    class TargetPose:
        ...
        class SetTargetToCharacter:
            ...
            #
            def set_character_byinput(self, input_pos: np.ndarray, input_quat : np.ndarray, other_character = None):
                if other_character is None:
                    other_character = self.character
                c_id = other_character.body_info.body_c_id
                other_character.world.loadBodyPos(c_id, input_pos.flatten())
                other_character.world.loadBodyQuat(c_id, input_quat.flatten())
...
class Render:
    ...
    class RenderWorld:
        ...
        def start_according_to_mask(self):
            if self.system:
                visDrawWorldYesRender()
            else:
                raise NotImplementedError
        ...
        def start_with_muscle_with_ref(self, muscle_hold_anchor_num, anchor_num):
            if self.system:
                visDrawWorldWithMuscleWithRef(muscle_hold_anchor_num, anchor_num) #)
            else:
                raise NotImplementedError
   
        
        def start_only_muscle_with_ref(self, muscle_hold_anchor_num, anchor_num):
            if self.system:
                visDrawWorldOnlyMuscleWithRef(muscle_hold_anchor_num, anchor_num) #)
            else:
                raise NotImplementedError

 
        def assign_muscle_property(self, muscle_activation, anchor_pos):
            if self.system:
                visAssignMuscleProperty(muscle_activation, anchor_pos) 
            else:
                raise NotImplementedError
        
        
        def assign_muscle_property_with_ref(self, muscle_activation, anchor_pos, ref_anchor_pos):
            if self.system:
                visAssignMusclePropertyWithRef(muscle_activation, anchor_pos, ref_anchor_pos) 
            else:
                raise NotImplementedError
