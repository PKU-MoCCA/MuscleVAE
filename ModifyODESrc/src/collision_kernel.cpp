...
void dxGeom::computePosr_pos_with_input(dMatrix3 input_rot, dVector3 input_pos, dVector3 res)
{
  // should only be recalced if we need to - ie offset from a body
  // dIASSERT(offset_posr);
  // std::cout << "test pos: " << body->posr.pos[0] << " " << body->posr.pos[1] << " " << body->posr.pos[2] << " " << input_pos[0] << " " << input_pos[1] << " " << input_pos[2] << std::endl;
  // std::cout << "test rot: " << body->posr.R[0] << " " << body->posr.R[1] << " " << body->posr.R[2] << " " << body->posr.R[4] << " " << body->posr.R[5] << " " << body->posr.R[6] << " " << body->posr.R[8] << " " << body->posr.R[9] << " " << body->posr.R[10] << " " << " " << input_rot[0] << " " << input_rot[1] << " " << input_rot[2] << " " << input_rot[4] << " " << input_rot[5] << " " << input_rot[6] << " " << input_rot[8] << " " << input_rot[9] << " " << input_rot[10]<< std::endl;
  // std::cout << "offset pos" << std::endl;
  // std::cout << offset_posr->pos[0] << " " << offset_posr->pos[1] << " " << offset_posr->pos[2] << " " << std::endl;
  // std::cout << "offset R" << std::endl;
  // std::cout << offset_posr->R[0] << " " << offset_posr->R[1] << " " << offset_posr->R[2] << " " << offset_posr->R[4] << " " << offset_posr->R[5] << " " << offset_posr->R[6] << " "<< offset_posr->R[8] << " " << offset_posr->R[9] << " " << offset_posr->R[10] << std::endl;
  // std::cout << "input pos" << std::endl;
  // std::cout << input_pos[0] << " " << input_pos[1] << " " << input_pos[2] << " " << std::endl;
  // std::cout << "input R" << std::endl;
  // std::cout << input_rot[0] << " " << input_rot[1] << " " << input_rot[2] << " " << input_rot[4] << " " << input_rot[5] << " " << input_rot[6] << " " << input_rot[8] << " " << input_rot[9] << " " << input_rot[10] << std::endl;

  dMultiply0_331(res, input_rot, offset_posr->pos);
  res[0] += input_pos[0];
  res[1] += input_pos[1];
  res[2] += input_pos[2];
}
...
void dxGeom::computePosr_r_with_input(dMatrix3 input_rot, dVector3 input_pos, dMatrix3 res)
{
  // should only be recalced if we need to - ie offset from a body
  // dIASSERT(offset_posr);
  dMultiply0_333(res, input_rot, offset_posr->R);
}
...
void dGeomGetPosition_with_input_matrix(dxGeom *g, dMatrix3 R, dVector3 pos, dVector3 res)
{
  // dAASSERT (g);
  // dUASSERT (g->gflags & GEOM_PLACEABLE,"geom must be placeable");
  return g->computePosr_pos_with_input(R, pos, res);
}


void dGeomGetRotation_with_input_matrix(dxGeom *g, dMatrix3 R, dVector3 pos, dMatrix3 res)
{
  // dAASSERT (g);
  // dUASSERT (g->gflags & GEOM_PLACEABLE,"geom must be placeable");
  return g->computePosr_r_with_input(R, pos, res);
}
