...
cdef extern from "ode/ode.h":
    ...
    void dBodySetWhetherRender (dBodyID b, int render_or_not)

    
    int dBodyGetWhetherRender (dBodyID b)
    ...
    # void dJointGetFixedAnchor (dJointID j, dVector3 result)
...
#    dSpaceID dHashSpaceCreate(int space)
    ...
    void dGeomGetPosition_with_input_matrix (dGeomID g, dMatrix3 R, dVector3 pos, dVector3 res)
    
    void dGeomGetRotation_with_input_matrix (dGeomID g, dMatrix3 R, dVector3 pos, dMatrix3 res)
...
cdef extern from "joint_local_quat_batch.h":
    ...
    void pd_control_batch_with_fatigue(
        dJointID* joints,
        int joint_count,
        const dReal* input_target_local_qs,
        const dReal* kps,
