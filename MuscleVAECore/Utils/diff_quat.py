...
class CatWithMask(Function):
    ...
    def backward(ctx: Any, *grad_outputs) -> Tuple[torch.Tensor, torch.Tensor, None]:
        ...
        idx1: torch.Tensor = ctx.saved_tensors[0] 
...
def quat_multiply_bst(p: torch.Tensor, q: torch.Tensor) -> torch.Tensor:
    """
    multiply 2 quaternions. 
    p.shape is (batch_sz, :, 4) 
    q.shape is (:, 4) 
    """
    assert len(p.shape) == 3 and p.shape[-1] == 4
    assert len(q.shape) == 2 and q.shape[-1] == 4
...
# Add test code for quat_multiply_bst
if __name__ == "__main__":
    a = np.random.rand(3)
    b = np.random.rand(3)
    vec_a = Rotation.from_rotvec(a)
    vec_b = Rotation.from_rotvec(b)
    quat_a = torch.from_numpy(vec_a.as_quat().reshape(-1,4))
    quat_a = torch.cat([quat_a, quat_a], dim=0)
    quat_b = torch.from_numpy(vec_b.as_quat().reshape(-1,4))
    quat_b = torch.cat([quat_b, quat_b], dim=0)
    quat_aa = quat_a.reshape(-1, 1, 4) 
    quat_aa =torch.cat([quat_aa, quat_aa], dim=0)
    # quat_bb = torch.cat([quat_b, quat_b], dim=0)

    ori = quat_multiply(quat_a, quat_b)
    print(quat_aa.shape)
    nxt = quat_multiply_bst(quat_aa, quat_b)
    print("ori", ori)
    print("nxt", nxt)